...
OUTPUT_ROOT="/projects/fluid-engine/out/step209_postpatch_omega_retrigger_local"
...
usage() {
  cat <<'USAGE'
Run Step-209 post-patch long-horizon omega retrigger sweep.
USAGE
}
...
run_case "patched_static_long" "0.0" "0.0"
run_case "patched_omega_2pi_long" "6.283185307179586" "12.566370614359172"
run_case "patched_omega_6pi_long" "18.84955592153876" "37.69911184307752"
...
python3 "${REPO_ROOT}/scripts/summarize_step209_postpatch_omega_retrigger.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --summary-csv "${OUTPUT_ROOT}_summary.csv" \
  --phase2-csv "${OUTPUT_ROOT}_phase2.csv" \
  --out-csv "${OUTPUT_ROOT}_omega_retrigger.csv" \
  --out-md "${OUTPUT_ROOT}_omega_retrigger.md"
...
printf 'omega_retrigger_md,%s\n' "${OUTPUT_ROOT}_omega_retrigger.md"
