...
OUTPUT_ROOT="/projects/fluid-engine/out/step146_continuity_projection_ab_local"
...
usage() {
  cat <<'USAGE'
Run Step-146 continuity-consistent current projection A/B matrix.

Goal:
  Per GPT-14/14.5 and Step-145 null-result sweep, test whether enforcing
  source-step continuity-consistent j0 projection moves the persistent
  full-branch mode-0 closure / transport / ledger blocker.

Case matrix:
  - baseline_projection_off
  - projection_on_controlled
  - projection_on_full
  - projection_on_both

Usage:
  ./scripts/run_step146_continuity_projection_ab.sh [options]
...
if [[ ! -f "${CONTROLLED_INPUT}" ]]; then
  echo "Controlled input missing: ${CONTROLLED_INPUT}" >&2
  exit 1
fi
if [[ ! -f "${FULL_INPUT}" ]]; then
  echo "Full input missing: ${FULL_INPUT}" >&2
  exit 1
fi
...
run_case() {
  ...
  local controlled_extra=()
  local full_extra=()
  for arg in "$@"; do
    case "${arg}" in
      controlled:*)
        controlled_extra+=("${arg#controlled:}")
        ;;
      full:*)
        full_extra+=("${arg#full:}")
        ;;
      both:*)
        controlled_extra+=("${arg#both:}")
        full_extra+=("${arg#both:}")
        ;;
      *)
        echo "Unknown run_case arg prefix: ${arg}" >&2
        exit 2
        ;;
    esac
  done
  ...
  for arg in "${FULL_PINNED_ARGS[@]}"; do
    cmd+=(--full-arg "${arg}")
  done
  for arg in "${controlled_extra[@]}"; do
    cmd+=(--controlled-arg "${arg}")
  done
  for arg in "${full_extra[@]}"; do
    cmd+=(--full-arg "${arg}")
  done
...
run_case "baseline_projection_off"
run_case "projection_on_controlled" \
  "controlled:modes4d/continuity_consistent_current_projection_enable=true"
run_case "projection_on_full" \
  "full:modes4d/continuity_consistent_current_projection_enable=true"
run_case "projection_on_both" \
  "both:modes4d/continuity_consistent_current_projection_enable=true"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_projection_off" \
...
python3 "${REPO_ROOT}/scripts/summarize_step140_mode0_term_isolation.py" \
  ...
  --min-max-jw-ew 1e-12 \
  --min-max-s-leak-abs 1e-12 \
  --min-max-emf-vw-c-abs 1e-4

python3 "${REPO_ROOT}/scripts/summarize_step146_continuity_projection_ab.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_projection_off" \
  --out-csv "${OUTPUT_ROOT}_continuity_projection.csv" \
  --out-md "${OUTPUT_ROOT}_continuity_projection.md"

echo ""
echo "Step-146 continuity projection A/B complete."
...
echo "continuity_projection_md,${OUTPUT_ROOT}_continuity_projection.md"
