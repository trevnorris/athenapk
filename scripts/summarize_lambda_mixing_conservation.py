#!/usr/bin/env python3
"""Summarize lambda-dynamic-mixing conservation A/B runs."""
...
def main():
        ...
        rows.append(
            {
                ...
                "response_lambda_dot_init": parse_arg_from_log(
                    log_path, "modes4d/response_lambda_dot_init"
                ),
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Lambda Dynamic Mixing Conservation A/B Summary\n\n")
        f.write(f"Parsed `{len(rows)}` cases from glob `{args.glob}`.\n\n")
        if not rows:
            f.write("No valid case outputs found.\n")
        else:
            f.write(
                "| case | lambda_dot_init | mix_gain | psi0 ratio | psi_w0-psi_proj | em_u_bulk | em_u_resolved | em_u_sub | mix_a_abs | mix_pi_abs | jw_ew | s_leak_abs | projected/local/transport/ledger |\n"
            )
            f.write(
                "|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|\n"
            )
            for r in rows:
                ...
                f.write(
                    ...
                    + " | ".join(
                        [
                            r["case_name"],
                            fmt(r["response_lambda_dot_init"]),
                            fmt(r["response_lambda_dynamic_mixing_gain"]),
                            ...
                            fmt(r["full_final_int_response_lambda_dynamic_mix_a_abs"]),
                            fmt(r["full_final_int_response_lambda_dynamic_mix_pi_abs"]),
