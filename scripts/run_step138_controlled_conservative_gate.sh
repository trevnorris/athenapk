...
OUTPUT_ROOT="/projects/fluid-engine/out/step138_controlled_conservative_gate_local"
...
usage() {
  cat <<'USAGE'
Run Step-138 controlled conservative-EM gate.

Goal:
  Re-run controlled boundedness gates with controlled branch forced onto the
  conservative EM transport path, while keeping the full branch anchored to
  the transfer-active configuration.

Usage:
  ./scripts/run_step138_controlled_conservative_gate.sh [options]
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "ctrl_baseline_reference" \
  --out-csv "${OUTPUT_ROOT}_solver_equivalence.csv" \
  --out-md "${OUTPUT_ROOT}_solver_equivalence.md"

echo ""
echo "Step-138 controlled conservative gate complete."
