#!/usr/bin/env python3
"""Compare local-gradient gradient-power proxies against diagonal (d_z a_w)^2 drift."""
...
def compute_rows(case_dir, cols, fractions):
    ...
    for mode in (1, 2):
        ...
        series_keys = {
            f"aw_mode{mode}_gradz_power_mix_w0_local_gradient": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_gradient",
            f"aw_mode{mode}_gradz_power_mix_w0_local_gradient_discrete": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_gradient_discrete",
            f"aw_mode{mode}_gradz_power_mix_w0_local_dz": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_dz",
            f"aw_mode{mode}_gradz_power_mix_w0_local_dz_discrete": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_dz_discrete",
            f"aw_mode{mode}_gradz_power_mix_a": f"m4d_int_aw_mode{mode}_gradz_power_mix_a",
            f"aw_mode{mode}_gradz_power_da_dt": f"m4d_int_aw_mode{mode}_gradz_power_da_dt",
        }
        ...
        for idx in range(len(fractions) - 1):
            ...
            for name, series in components.items():
                ...
                alpha = fit_single(target_window, window_vals)
                if math.isfinite(alpha):
                    corrected = [y - alpha * x for y, x in zip(target_window, window_vals)]
...
def main():
    ...
    if not rows:
        raise SystemExit("No local-gradient discrete-power rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-173 Local-Gradient Discrete Power Probe\n\n")
