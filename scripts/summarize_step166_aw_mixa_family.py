#!/usr/bin/env python3
"""Summarize per-mode A_w mix_a gradient-power families against the total mix_a gradient-power target."""
...
def compute_rows(case_dir, cols, fractions):
    ...
    for mode in (1, 2):
        target_key = f"m4d_int_aw_mode{mode}_gradz_power_mix_a"
        ...
        series_keys = {
            f"aw_mode{mode}_gradz_power_mix_w0_dynamic": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_dynamic",
            f"aw_mode{mode}_gradz_power_mix_w0_local_gradient": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_gradient",
            f"aw_mode{mode}_gradz_power_mix_lambda": f"m4d_int_aw_mode{mode}_gradz_power_mix_lambda",
        }
            ...
            rows.append(
                {
                    ...
                    "mode": mode,
                    "window": window,
                    "component": f"d_aw_mode{mode}_gradz_power_mix_a_dt",
...
def main():
    ...
    if not rows:
        raise SystemExit("No A_w mix_a family rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-166 A_w mix_a Family Gradient-Power Audit\n\n")
        ...
        for key in sorted(grouped.keys(), key=lambda x: (x[0], x[1], window_order.get(x[2], 99))):
            ...
            grp = grouped[key]
            target = next((r for r in grp if r["component"] == f"d_aw_mode{mode}_gradz_power_mix_a_dt"), None)
            f.write(f"## {case_name} mode={mode} window={window}\n\n")
            if target:
                f.write(f"- target_rms_abs_rate: `{fmt(target['rms_abs_rate'])}`\n\n")
            ranked = [r for r in grp if r["component"] != f"d_aw_mode{mode}_gradz_power_mix_a_dt"]
