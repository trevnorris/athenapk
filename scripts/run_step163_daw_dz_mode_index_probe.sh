...
OUTPUT_ROOT="/projects/fluid-engine/out/step163_daw_dz_mode_index_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-163 d_z a_w mode-index probe on the corrected quadrature baseline.

Goal:
  Split the dominant diagonal (d_z a_w)^2 channel by EM mode index to identify
  which actual mode carries the remaining ledger drift.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step163_daw_dz_mode_index_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step163_daw_dz_mode_index.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_daw_dz_mode_index.csv" \
  --out-md "${OUTPUT_ROOT}_daw_dz_mode_index.md"
...
echo "daw_dz_mode_index_md,${OUTPUT_ROOT}_daw_dz_mode_index.md"
