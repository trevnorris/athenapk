...
OUTPUT_ROOT="/projects/fluid-engine/out/step164_aw_mode_bookkeeping_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-164 A_w mode-bookkeeping probe on the corrected quadrature baseline.

Goal:
  Compare the dominant per-mode (d_z a_w)^2 energy drifts against the signed A_w
  update bookkeeping pieces: pi_drive, rhs_aw, mix_piw, mix_aw, and total dA_w/dt.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step164_aw_mode_bookkeeping_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step164_aw_mode_bookkeeping.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_mode_bookkeeping.csv" \
  --out-md "${OUTPUT_ROOT}_aw_mode_bookkeeping.md"
...
echo "aw_mode_bookkeeping_md,${OUTPUT_ROOT}_aw_mode_bookkeeping.md"
