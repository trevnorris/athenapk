#!/usr/bin/env python3
"""Compare exact discrete Cx/Cz energy increments against the bulk residual."""
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        x_exact = select_window(
            t_r,
            derivative_series(times, cols["m4d_int_cx_energy_delta_exact_discrete_sum"])[1],
            t_lo,
            end,
            include_right=True,
        )
        z_exact = select_window(
            t_r,
            derivative_series(times, cols["m4d_int_cz_energy_delta_exact_discrete_sum"])[1],
            t_lo,
            end,
            include_right=True,
        )
        xz_exact = sum_series(x_exact, z_exact)

        components = {
            "cx_exact_sum": x_exact,
            "cz_exact_sum": z_exact,
    ...
    if not rows:
        raise SystemExit("No Step-190 rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-190 Cx/Cz Exact Delta Probe\n\n")
        ...
        for case_name in sorted({r['case_name'] for r in rows}):
            ...
            target = next(
                r for r in rows if r["case_name"] == case_name and r["candidate"] == "em_bulk_ledger_residual"
            )
