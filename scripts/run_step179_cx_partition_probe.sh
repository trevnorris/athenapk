...
OUTPUT_ROOT="/projects/fluid-engine/out/step179_cx_partition_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-179 Cx partition probe on the corrected quadrature baseline.

Goal:
  Decompose the surviving Cx^2 bookkeeping channel into:
  - daw_dx^2
  - dw_ax^2
  - cross term
...
Usage:
  ./scripts/run_step179_cx_partition_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step179_cx_partition.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_cx_partition.csv" \
  --out-md "${OUTPUT_ROOT}_cx_partition.md"
...
echo "cx_partition_md,${OUTPUT_ROOT}_cx_partition.md"
