...
OUTPUT_ROOT="/projects/fluid-engine/out/step193_exact_current_cxz_equivalence_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-193 exact-current Cx/Cz bulk-energy equivalence probe.

Goal:
  Compare the current EM bulk-energy history against an alternative bulk-energy
  history that replaces continuum Cx/Cz state channels with exact-current host
  bookkeeping channels.

Usage:
  ./scripts/run_step193_exact_current_cxz_equivalence_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step193_exact_current_cxz_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_exact_current_cxz_equivalence.csv" \
  --out-md "${OUTPUT_ROOT}_exact_current_cxz_equivalence.md"
...
printf 'exact_current_cxz_equivalence_md,%s\n' "${OUTPUT_ROOT}_exact_current_cxz_equivalence.md"
