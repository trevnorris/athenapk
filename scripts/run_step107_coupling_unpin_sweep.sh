...
OUTPUT_ROOT="/projects/fluid-engine/out/step107_coupling_unpin_sweep_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-107 coupling-unpin sweep.

Goal:
  Identify whether source-gain couplings are pinning JwEw near zero despite
  transverse channel activation.

Usage:
  ./scripts/run_step107_coupling_unpin_sweep.sh [options]
...
# 0) reference (combined-forced from Step-106)
run_case "combined_ref"

# 1) stronger EM current + timelike coupling
run_case "em_cur_timelike_x2" \
  "modes4d/em_source_current_gain=2.0" \
  "modes4d/em_source_timelike_gain=2.0"

# 2) aggressive EM current + timelike coupling
run_case "em_cur_timelike_x4" \
  "modes4d/em_source_current_gain=4.0" \
  "modes4d/em_source_timelike_gain=4.0"

# 3) stronger plasma momw couplings
run_case "momw_force_x2" \
  "modes4d/plasma_momw_source_gain=2.0" \
  "modes4d/plasma_momw_pressure_source_gain=2.0"

# 4) moderate all-channel boost with lower damping
run_case "all_x2_lowdamp" \
  "modes4d/em_source_current_gain=2.0" \
  "modes4d/em_source_timelike_gain=2.0" \
  "modes4d/plasma_momw_source_gain=2.0" \
  "modes4d/plasma_momw_pressure_source_gain=2.0" \
  "modes4d/em_source_damping_gain=0.5"

# 5) aggressive all-channel boost with lower damping
run_case "all_x4_lowdamp" \
  "modes4d/em_source_current_gain=4.0" \
  "modes4d/em_source_timelike_gain=4.0" \
...
echo ""
echo "Step-107 coupling-unpin sweep complete."
