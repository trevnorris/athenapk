...
OUTPUT_ROOT="/projects/fluid-engine/out/step166_aw_mixa_family_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-166 A_w mix_a family gradient-power probe on the corrected quadrature baseline.

Goal:
  Split the dominant gradient-weighted A_w power channel by mix_a family
  (w0_dynamic, w0_local_gradient, lambda) for modes 1 and 2.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step166_aw_mixa_family_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step166_aw_mixa_family.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_mixa_family.csv" \
  --out-md "${OUTPUT_ROOT}_aw_mixa_family.md"
...
echo "aw_mixa_family_md,${OUTPUT_ROOT}_aw_mixa_family.md"
