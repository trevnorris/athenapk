...
OUTPUT_ROOT="/projects/fluid-engine/out/step105_scalar_channel_activation_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-105 scalar/mixed-channel activation probe.

Goal:
  Force activation of mixed-sector channels (Aw/Piw and Ay/Piy mode-1 seeds)
  and test whether sustained Jw/leakage/EMF transfer appears.

Usage:
  ./scripts/run_step105_scalar_channel_activation_probe.sh [options]
...
# 1) Aw/Piw boosted (scalar-photon channel emphasis)
run_case "awpiw_boost" \
  "problem/harris_4d/aw_mode1_amp=2.0e-1" \
  "problem/harris_4d/piw_mode1_amp=1.0e-1"

# 2) Ay/Piy boosted (mixed C_a pathway emphasis)
run_case "aypiy_boost" \
  "problem/harris_4d/ay_mode1_amp=1.0e-1" \
  "problem/harris_4d/piy_mode1_amp=5.0e-2"

# 3) combined channel + small Jw seed
run_case "combined_plus_jwseed" \
  "problem/harris_4d/aw_mode1_amp=2.0e-1" \
  "problem/harris_4d/piw_mode1_amp=1.0e-1" \
  "problem/harris_4d/ay_mode1_amp=1.0e-1" \
  "problem/harris_4d/piy_mode1_amp=5.0e-2" \
  "problem/harris_4d/drift_momw_scale=2.0e-2"

python3 "${REPO_ROOT}/scripts/summarize_scalar_channel_activation.py" \
  --glob "${OUTPUT_ROOT}/*" \
...
echo ""
echo "Step-105 scalar-channel activation probe complete."
