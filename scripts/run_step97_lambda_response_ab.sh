...
OUTPUT_ROOT="/projects/fluid-engine/out/step97_lambda_response_ab_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-97 lambda-response A/B sweep.

Cases:
  - baseline_fixed_center_t0p6
  - lambda_g1_t0p6
  - lambda_g3_t0p6
  - lambda_g3_bridge0p1_t0p6
  - lambda_g3_bridge0p1_t1p2

Usage:
  ./scripts/run_step97_lambda_response_ab.sh [options]
...
run_case "lambda_g1_t0p6" "${SHORT_TLIM}" "${SHORT_NLIM}" \
  "modes4d/response_lambda_enable=true" \
  "modes4d/response_lambda_drive_gain=1.0" \
...
run_case "lambda_g3_t0p6" "${SHORT_TLIM}" "${SHORT_NLIM}" \
  "modes4d/response_lambda_enable=true" \
...
run_case "lambda_g3_bridge0p1_t0p6" "${SHORT_TLIM}" "${SHORT_NLIM}" \
  "modes4d/response_lambda_enable=true" \
  "modes4d/response_lambda_drive_gain=3.0" \
  "modes4d/response_lambda_mass=0.20" \
  "modes4d/response_lambda_stiffness=0.10" \
  ...
  "modes4d/response_lambda_drive_from_bridge_gain=0.10" \
  "modes4d/response_lambda_drive_from_bridge_channel=primary" \
  "modes4d/response_lambda_drive_from_bridge_abs=false"

run_case "lambda_g3_bridge0p1_t1p2" "${LONG_TLIM}" "${LONG_NLIM}" \
  "modes4d/response_lambda_enable=true" \
  "modes4d/response_lambda_drive_gain=3.0" \
  "modes4d/response_lambda_mass=0.20" \
  "modes4d/response_lambda_stiffness=0.10" \
...
echo ""
echo "Step-97 lambda-response A/B campaign complete."
