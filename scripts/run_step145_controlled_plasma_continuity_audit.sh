...
OUTPUT_ROOT="/projects/fluid-engine/out/step145_controlled_plasma_continuity_audit_local"
...
usage() {
  cat <<'USAGE'
Run Step-145 controlled plasma continuity-coupling audit.

Goal:
  Follow GPT-14/14.5 decision tree after Step-144 PASS:
  isolate plasma-coupled current/continuity controls in the controlled branch
  while keeping the full branch pinned to transfer-active settings.
...
Case matrix:
  ...
  - c3_current_off
  - c3_current_hi
  - c3_current_off_rho_divj_hi
  - c3_rusanov_off

Usage:
  ./scripts/run_step145_controlled_plasma_continuity_audit.sh [options]
...
run_case "c3_baseline_strict"
run_case "c3_rho_divj_off" "modes4d/plasma_rho_divj_gain=0.0"
run_case "c3_rho_divj_hi" "modes4d/plasma_rho_divj_gain=5.0"
run_case "c3_current_off" "modes4d/em_source_current_gain=0.0"
run_case "c3_current_hi" "modes4d/em_source_current_gain=14.0"
run_case "c3_current_off_rho_divj_hi" \
  "modes4d/em_source_current_gain=0.0" \
  "modes4d/plasma_rho_divj_gain=5.0"
run_case "c3_rusanov_off" "modes4d/plasma_pressure_rusanov_gain=0.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "c3_baseline_strict" \
...
python3 "${REPO_ROOT}/scripts/summarize_step145_plasma_continuity_coupling.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "c3_baseline_strict" \
  --controlled-bounds-csv "${OUTPUT_ROOT}_controlled_bounds.csv" \
  --out-csv "${OUTPUT_ROOT}_continuity_coupling.csv" \
  --out-md "${OUTPUT_ROOT}_continuity_coupling.md"

echo ""
echo "Step-145 controlled plasma continuity-coupling audit complete."
...
echo "continuity_coupling_md,${OUTPUT_ROOT}_continuity_coupling.md"
