#!/usr/bin/env python3
"""Summarize long-horizon post-patch omega retrigger sweep."""
...
def main():
    ...
    for case_dir in [Path(p) for p in sorted(glob.glob(args.glob)) if Path(p).is_dir()]:
        ...
        max_c2, t_c2 = max_abs_with_time(times, full["m4d_mixed_c2"])
        late_c2, t_late_c2 = max_abs_with_time(times, full["m4d_mixed_c2"], tmin=0.5)
        max_emf, t_emf = max_abs_with_time(times, full["m4d_emf_vw_c_abs"])
        late_emf, t_late_emf = max_abs_with_time(times, full["m4d_emf_vw_c_abs"], tmin=0.5)
        ...
        row = {
            ...
            "late_max_abs_emf_vw_c_abs": late_emf,
            "time_at_late_max_abs_emf_vw_c_abs": t_late_emf,
    ...
    if not rows:
        raise SystemExit("No Step-209 rows computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-209 Post-Patch Omega Retrigger Sweep\n\n")
        f.write("| case | psi0 ratio | final gap | late max gap | late gap rms | postburst peaks | late max C2 | late max EMF | proj/local/transport/ledger |\n")
        f.write("|:---|---:|---:|---:|---:|---:|---:|---:|:---|\n")
        for r in rows:
            f.write(
                ...
                f"{fmt(r['late_max_abs_mixed_c2'])} | {fmt(r['late_max_abs_emf_vw_c_abs'])} | "
        ...
        for r in rows:
            ...
            f.write(f"- late max `|emf_vw_c_abs|`: `{fmt(r['late_max_abs_emf_vw_c_abs'])}` at `t={fmt(r['time_at_late_max_abs_emf_vw_c_abs'])}`\n")
