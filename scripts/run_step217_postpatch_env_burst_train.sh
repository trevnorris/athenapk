...
OUTPUT_ROOT="/projects/fluid-engine/out/step217_postpatch_env_burst_train_local"
...
usage() {
  cat <<'USAGE'
Run Step-217 post-patch envelope-modulated burst-train forcing sweep.
USAGE
}
...
run_case "patched_env_train_triplet_amp0005" "6.25e-5" "3.75e-5" "2.0e-1" "3.5e-1" "2.0e-2"
run_case "patched_env_train_triplet_amp0010" "1.25e-4" "7.5e-5" "2.0e-1" "3.5e-1" "2.0e-2"
run_case "patched_env_train_quad_amp0005" "6.25e-5" "3.75e-5" "1.5e-1" "4.0e-1" "1.5e-2"
run_case "patched_env_train_quad_amp0010" "1.25e-4" "7.5e-5" "1.5e-1" "4.0e-1" "1.5e-2"
...
python3 "${REPO_ROOT}/scripts/summarize_step217_postpatch_env_burst_train.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --summary-csv "${OUTPUT_ROOT}_summary.csv" \
  --phase2-csv "${OUTPUT_ROOT}_phase2.csv" \
  --out-csv "${OUTPUT_ROOT}_env_burst_train.csv" \
  --out-md "${OUTPUT_ROOT}_env_burst_train.md"
