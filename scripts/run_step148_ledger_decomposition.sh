...
OUTPUT_ROOT="/projects/fluid-engine/out/step148_ledger_decomposition_local"
...
usage() {
  cat <<'USAGE'
Run Step-148 transport/ledger decomposition matrix.

Goal:
  With full-branch continuity projection fixed ON (Step-146 result), isolate
  remaining transport/ledger residuals across conservative bookkeeping families.
...
Case matrix:
  ...
  - div_transport_off
  - conservative_off
  - conservative_off_em_sources_off

Usage:
  ./scripts/run_step148_ledger_decomposition.sh [options]
...
Options:
  ...
  --resume BOOL            Skip already PASS cases (default: true)
  -h, --help               Show help
USAGE
}
...
run_case "baseline_decomp"
run_case "em_sources_off" \
  "modes4d/em_source_mass_gain=0.0" \
  "modes4d/em_source_laplacian_gain=0.0" \
  "modes4d/em_source_current_gain=0.0" \
  "modes4d/em_source_timelike_gain=0.0" \
  "modes4d/em_source_damping_gain=0.0"
run_case "plasma_sources_off" \
...
python3 "${REPO_ROOT}/scripts/summarize_step148_ledger_decomposition.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_decomp" \
  --out-csv "${OUTPUT_ROOT}_decomposition.csv" \
  --out-md "${OUTPUT_ROOT}_decomposition.md"

echo ""
echo "Step-148 ledger decomposition complete."
echo "output_root,${OUTPUT_ROOT}"
echo "decomposition_md,${OUTPUT_ROOT}_decomposition.md"
