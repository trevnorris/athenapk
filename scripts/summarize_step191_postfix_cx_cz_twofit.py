#!/usr/bin/env python3
"""Two-channel exact Cx/Cz fit on the post-fix Step-190 rerun."""
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        candidates = {
            "exact_cx_cz_twofit": (x, z),
            "exact_cz_only": ([0.0] * len(z), z),
            "exact_cx_only": (x, [0.0] * len(x)),
        }
        for name, (x_series, z_series) in candidates.items():
            coeff_x, coeff_z = fit_two(y, x_series, z_series)
            corrected = [
                yy - coeff_x * xx - coeff_z * zz
                for yy, xx, zz in zip(y, x_series, z_series)
            ]
            _, corrected_rms, _ = finite_stats(corrected)
            reduction = res_rms / corrected_rms if corrected_rms > 0 else math.inf
            modeled = [coeff_x * xx + coeff_z * zz for xx, zz in zip(x_series, z_series)]
            rows.append(
                {
                    "case_name": case_dir.name,
                    "component": name,
                    "coeff_x": coeff_x,
                    "coeff_z": coeff_z,
                    "corr_x": pearson(y, x_series),
                    "corr_z": pearson(y, z_series),
                    "rms_abs_rate": finite_stats(modeled)[1],
                    "max_abs_rate": finite_stats(modeled)[0],
                    "final_rate": modeled[-1] if modeled else math.nan,
    ...
    if not rows:
        raise SystemExit("No Step-191 rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-191 Post-Fix Exact Cx/Cz Two-Channel Fit\n\n")
        ...
        for case_name in sorted({r["case_name"] for r in rows}):
            ...
            target = next(
                r
                for r in rows
                if r["case_name"] == case_name and r["component"] == "em_bulk_ledger_residual"
            )
            f.write(f"- residual_rms_abs_rate: `{fmt(target['rms_abs_rate'])}`\n\n")
            f.write(
                "| component | coeff_x | coeff_z | corr_x | corr_z | corrected_rms_abs_rate | rms_reduction_factor | rms_abs_rate | max_abs_rate | final_rate |\n"
            )
            f.write("|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
            ranked = [
                r
            ...
            for r in ranked:
                f.write(
                    f"| {r['component']} | {fmt(r['coeff_x'])} | {fmt(r['coeff_z'])} | {fmt(r['corr_x'])} | {fmt(r['corr_z'])} | {fmt(r['corrected_rms_abs_rate'])} | {fmt(r['rms_reduction_factor'])} | {fmt(r['rms_abs_rate'])} | {fmt(r['max_abs_rate'])} | {fmt(r['final_rate'])} |\n"
                )
            f.write("\n")
