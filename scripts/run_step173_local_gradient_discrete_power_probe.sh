...
OUTPUT_ROOT="/projects/fluid-engine/out/step173_local_gradient_discrete_power_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-173 local-gradient discrete power probe on the corrected quadrature baseline.

Goal:
  Compare the surviving per-mode (d_z a_w)^2 drift against new discrete z-gradient power
  proxies for the dominant w0_local_gradient -> mix_a path.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step173_local_gradient_discrete_power_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step173_local_gradient_discrete_power.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_local_gradient_discrete_power.csv" \
  --out-md "${OUTPUT_ROOT}_local_gradient_discrete_power.md"
...
echo "local_gradient_discrete_power_md,${OUTPUT_ROOT}_local_gradient_discrete_power.md"
