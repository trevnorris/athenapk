...
OUTPUT_ROOT="/projects/fluid-engine/out/step222_dynamic_response_state_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-222 dynamic response-state onset probe.
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step222_dynamic_response_state_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --summary-csv "${OUTPUT_ROOT}_summary.csv" \
  --phase2-csv "${OUTPUT_ROOT}_phase2.csv" \
  --gate-on "${GATE_T_ON}" \
  --gate-off "${GATE_T_OFF}" \
  --out-csv "${OUTPUT_ROOT}_dynamic_response_state.csv" \
  --out-md "${OUTPUT_ROOT}_dynamic_response_state.md"

printf 'summary_csv,%s\n' "${OUTPUT_ROOT}_mode0_terms.csv"
...
printf 'dynamic_response_state_md,%s\n' "${OUTPUT_ROOT}_dynamic_response_state.md"
