...
OUTPUT_ROOT="/projects/fluid-engine/out/step141_transform_projection_ab_local"
...
usage() {
  cat <<'USAGE'
Run Step-141 transform/projection A/B with preflight integrity checks.

Plan:
  1) Run transform-integrity suite (Step-131) in this output bundle.
  2) Run Harris A/B with identical physics knobs and conservative controlled path:
     - baseline projection
     - mass-matrix (Gram-inverse) projection enabled
  3) Summarize closure/ledger/activation deltas.

Usage:
  ./scripts/run_step141_transform_projection_ab.sh [options]
...
Options:
  ...
  --test-build-dir DIR     Build dir for transform audit binaries
  --jobs N                 Build jobs for transform suite
...
TRANSFORM_OUT="${OUTPUT_ROOT}/transform_suite"
"${REPO_ROOT}/scripts/run_step131_transform_integrity_suite.sh" \
  --binary "${BINARY}" \
  --test-build-dir "${TEST_BUILD_DIR}" \
  --workdir "${WORKDIR}" \
...
run_case() {
  ...
  for arg in "${CONTROLLED_BOUND_ARGS[@]}"; do
...
run_case "ab_projection_baseline" \
  "modes4d/mass_matrix_projection_enable=false"

run_case "ab_projection_mass_matrix" \
  "modes4d/mass_matrix_projection_enable=true"

python3 "${REPO_ROOT}/scripts/summarize_transverse_channel_activation.py" \
  --glob "${OUTPUT_ROOT}/ab_projection_*" \
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/ab_projection_*" \
  --baseline-case "ab_projection_baseline" \
...
echo ""
echo "Step-141 transform/projection A/B complete."
echo "output_root,${OUTPUT_ROOT}"
echo "transform_suite_md,${TRANSFORM_OUT}/transform_integrity_summary.md"
