...
OUTPUT_ROOT="/projects/fluid-engine/out/step153_pi_source_partition_local"
...
usage() {
  cat <<'USAGE'
Run Step-153 EM pi source partition probe.

Goal:
  Rerun the Step-150 Harris full baseline with the new split EM mode-0 `pi`
  diagnostics so we can determine whether the legacy `int_srcpi*` transport
  driver is dominated by the explicit RHS path or by dynamic-basis mixing.

Case matrix:
  - baseline_pi_partition
  - no_a0_mode2

Usage:
  ./scripts/run_step153_pi_source_partition.sh [options]
...
run_case "baseline_pi_partition"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_pi_partition" \
...
python3 "${REPO_ROOT}/scripts/summarize_step153_pi_source_partition.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_pi_partition" \
  --out-csv "${OUTPUT_ROOT}_pi_source_partition.csv" \
  --summary-csv "${OUTPUT_ROOT}_pi_source_partition_channels.csv" \
  --out-md "${OUTPUT_ROOT}_pi_source_partition.md"

echo ""
echo "Step-153 EM pi source partition probe complete."
...
echo "pi_source_partition_md,${OUTPUT_ROOT}_pi_source_partition.md"
