...
OUTPUT_ROOT="/projects/fluid-engine/out/step139_full_closure_ledger_hardening_local"
...
usage() {
  cat <<'USAGE'
Run Step-139 full-branch closure/ledger hardening sweep.

Goal:
  Keep controlled branch in bounded conservative mode (from Step-138), and
  sweep full-branch forcing/damping knobs to improve:
    - local mode-0 closure status
    - transport closure status
    - EM bulk ledger status
  while preserving transfer activation floors.

Usage:
  ./scripts/run_step139_full_closure_ledger_hardening.sh [options]
...
run_case "full_baseline_consctrl"

run_case "full_gain10" \
  "modes4d/em_source_current_gain=10.0" \
  "modes4d/em_source_timelike_gain=10.0" \
  "modes4d/plasma_momw_source_gain=10.0" \
  "modes4d/plasma_momw_pressure_source_gain=10.0"

run_case "full_gain8_damp05" \
  ...
  "modes4d/em_source_damping_gain=5.0e-1" \
  "modes4d/em_damping=5.0e-2"

run_case "full_gain8_rho_divj2" \
  "modes4d/em_source_current_gain=8.0" \
  "modes4d/em_source_timelike_gain=8.0" \
  "modes4d/plasma_momw_source_gain=8.0" \
  "modes4d/plasma_momw_pressure_source_gain=8.0" \
  "modes4d/plasma_rho_divj_gain=2.0"

run_case "full_gain8_combo" \
  "modes4d/em_source_current_gain=8.0" \
  "modes4d/em_source_timelike_gain=8.0" \
  "modes4d/plasma_momw_source_gain=8.0" \
  "modes4d/plasma_momw_pressure_source_gain=8.0" \
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "full_baseline_consctrl" \
  --out-csv "${OUTPUT_ROOT}_solver_equivalence.csv" \
  --out-md "${OUTPUT_ROOT}_solver_equivalence.md"

echo ""
echo "Step-139 full closure/ledger hardening sweep complete."
