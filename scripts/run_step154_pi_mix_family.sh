...
OUTPUT_ROOT="/projects/fluid-engine/out/step154_pi_mix_family_local"
...
usage() {
  cat <<'USAGE'
Run Step-154 EM pi mixing-family probe.

Goal:
  Follow Step-153 by splitting the mode-0 `pi` mixing path into
  `w0_dynamic`, `w0_local_gradient`, and `lambda` families on the focused
  Harris baseline.
...
Usage:
  ./scripts/run_step154_pi_mix_family.sh [options]
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_mix_family" \
...
python3 "${REPO_ROOT}/scripts/summarize_step154_pi_mix_family.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_mix_family" \
  --out-csv "${OUTPUT_ROOT}_pi_mix_family.csv" \
  --summary-csv "${OUTPUT_ROOT}_pi_mix_family_channels.csv" \
  --out-md "${OUTPUT_ROOT}_pi_mix_family.md"

echo ""
echo "Step-154 EM pi mixing-family probe complete."
...
echo "pi_mix_family_md,${OUTPUT_ROOT}_pi_mix_family.md"
