...
OUTPUT_ROOT="/projects/fluid-engine/out/step207_postpatch_physics_baseline_local"
...
usage() {
  cat <<'USAGE'
Run Step-207 post-patch physics baseline on the live z-block-corrected branch.
USAGE
}
...
CASE_DIR="${OUTPUT_ROOT}/patched_physics_baseline"
...
if [[ "${RESUME}" == "true" ]] && [[ -f "${EXIT_FILE}" ]]; then
  prior_rc="$(<"${EXIT_FILE}")"
  if [[ "${prior_rc}" == "0" ]]; then
    echo "=== patched_physics_baseline ===" | tee -a "${LOG_FILE}"
    echo "case_status,patched_physics_baseline,SKIP,resume_hit" | tee -a "${LOG_FILE}"
  else
    rm -f "${EXIT_FILE}"
  fi
fi
...
if [[ ! -f "${EXIT_FILE}" ]]; then
  ...
  echo "=== patched_physics_baseline ===" | tee "${LOG_FILE}"
  ...
  echo "${rc}" > "${EXIT_FILE}"
  if [[ ${rc} -eq 0 ]]; then
    echo "case_status,patched_physics_baseline,PASS" | tee -a "${LOG_FILE}"
  else
    echo "case_status,patched_physics_baseline,FAIL,rc=${rc}" | tee -a "${LOG_FILE}"
    exit ${rc}
  fi
fi
