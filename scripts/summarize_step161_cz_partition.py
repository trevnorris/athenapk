#!/usr/bin/env python3
"""Summarize Cz partition of the mixed-C drift."""
...
def compute_rows(case_dir, cols, fractions):
    times = cols["time"]
    t_r, cz_rate = derivative_series(times, cols["m4d_mixed_cz2"])
    _, bulk_rate = derivative_series(times, cols["m4d_em_u_bulk"])
    if not t_r or len(cz_rate) != len(bulk_rate):
        return []

    component_rates = {
        "d_mixed_cz_daw_dz2_dt": derivative_series(times, cols["m4d_mixed_cz_daw_dz2"])[1],
    ...
    if any(len(v) != len(cz_rate) for v in component_rates.values()):
        return []
    ...
    for idx in range(len(fractions) - 1):
        ...
        cz_window = select_window(t_r, cz_rate, t_lo, t_hi, include_right)
        bulk_max, bulk_rms, _ = finite_stats(bulk_window)
        cz_max, cz_rms, cz_final = finite_stats(cz_window)
        if not cz_window:
            continue
        ...
        rows.append(
            {
                ...
                "window": window,
                "component": "d_mixed_cz2_dt",
                "max_abs_rate": cz_max,
                "rms_abs_rate": cz_rms,
                "final_rate": cz_final,
                "share_of_cz_rms": 1.0,
                "share_of_bulk_rms": cz_rms / bulk_rms if bulk_rms > 0.0 else math.nan,
                "corr_with_cz": 1.0,
                "corr_with_bulk": pearson(cz_window, bulk_window),
                "cz_rms_abs_rate": cz_rms,
            ...
            rows.append(
                {
                    ...
                    "share_of_cz_rms": rms_abs / cz_rms if cz_rms > 0.0 else math.nan,
                    "share_of_bulk_rms": rms_abs / bulk_rms if bulk_rms > 0.0 else math.nan,
                    "corr_with_cz": pearson(window_vals, cz_window),
...
def main():
    ...
    if not rows:
        raise SystemExit("No Cz partition rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-161 Cz Partition Audit\n\n")
        ...
        for (case_name, window) in sorted(grouped.keys(), key=lambda x: (x[0], window_order.get(x[1], 99))):
            ...
            cz = next((r for r in grp if r["component"] == "d_mixed_cz2_dt"), None)
            f.write(f"## {case_name} window={window}\n\n")
            if cz is not None:
                f.write(f"- mixed_cz_rms_abs_rate: `{fmt(cz['rms_abs_rate'])}`\n")
                f.write(f"- bulk_rms_abs_rate: `{fmt(cz['bulk_rms_abs_rate'])}`\n\n")
            ranked = [r for r in grp if r["component"] != "d_mixed_cz2_dt"]
            ranked.sort(key=lambda r: abs(r["share_of_cz_rms"]) if math.isfinite(r["share_of_cz_rms"]) else -1.0, reverse=True)
            f.write("| component | rms_abs_rate | share_of_cz_rms | share_of_bulk_rms | corr_with_cz | corr_with_bulk | max_abs_rate | final_rate |\n")
            f.write("|:---|---:|---:|---:|---:|---:|---:|---:|\n")
            for row in ranked:
                f.write(
                    f"| {row['component']} | {fmt(row['rms_abs_rate'])} | {fmt(row['share_of_cz_rms'])} | {fmt(row['share_of_bulk_rms'])} | {fmt(row['corr_with_cz'])} | {fmt(row['corr_with_bulk'])} | {fmt(row['max_abs_rate'])} | {fmt(row['final_rate'])} |\n"
                )
            f.write("\n")
