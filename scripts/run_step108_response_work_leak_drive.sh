...
OUTPUT_ROOT="/projects/fluid-engine/out/step108_response_work_leak_drive_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-108 response work/leak drive probe.

Goal:
  Test the next structural improvement after Step-107 by wiring dynamic response
  drives to measured transfer channels (JwEw and S_leak) and checking whether
  this unpins sustained spillback signatures.

Usage:
  ./scripts/run_step108_response_work_leak_drive.sh [options]
...
# 0) reference (Step-107 strongest coupling case family)
run_case "combined_ref"

# 1) lambda response driven directly from work/leak channels
run_case "lambda_work_leak_abs" \
  "modes4d/response_lambda_drive_gain=0.0" \
  "modes4d/response_lambda_drive_from_work_gain=2.0e4" \
  "modes4d/response_lambda_drive_from_work_abs=true" \
  "modes4d/response_lambda_drive_from_leak_gain=2.0e3" \
  "modes4d/response_lambda_drive_from_leak_abs=true"

# 2) w0 response driven directly from work/leak channels + geometry coupling
run_case "w0_work_leak_abs" \
  "modes4d/response_w0_enable=true" \
  "modes4d/response_w0_drive_gain=0.0" \
  "modes4d/response_w0_drive_from_work_gain=2.0e4" \
  "modes4d/response_w0_drive_from_work_abs=true" \
  "modes4d/response_w0_drive_from_leak_gain=2.0e3" \
  "modes4d/response_w0_drive_from_leak_abs=true" \
  "modes4d/response_w0_mass=0.20" \
  "modes4d/response_w0_stiffness=0.10" \
  "modes4d/response_w0_damping=0.05" \
  "modes4d/response_w0_max_abs=3.5e-1" \
  "modes4d/response_w0_geometry_shift_enable=true" \
  "modes4d/response_w0_geometry_shift_gain=1.0" \
  "modes4d/response_w0_lambda_shift_enable=true" \
  "modes4d/response_w0_lambda_shift_gain=2.5e-1" \
  "modes4d/response_w0_lambda_shift_max_frac=2.0e-1"

# 3) combined lambda+w0 work/leak driven response
run_case "w0_lambda_work_leak_abs" \
  "modes4d/response_lambda_drive_gain=0.0" \
  "modes4d/response_lambda_drive_from_work_gain=2.0e4" \
  "modes4d/response_lambda_drive_from_work_abs=true" \
  "modes4d/response_lambda_drive_from_leak_gain=2.0e3" \
...
echo ""
echo "Step-108 response work/leak drive probe complete."
