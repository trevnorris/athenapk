#!/usr/bin/env python3
"""Summarize the local-gradient mix_a structure against the total local-gradient mix_a target."""
...
def compute_rows(case_dir, cols, fractions):
    ...
    rows = []
    for mode in (1, 2):
        target_key = f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_gradient"
        ...
        if span <= 0.0:
            continue
        series_keys = {
            f"aw_mode{mode}_gradz_power_mix_w0_local_mode1": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_mode1",
            f"aw_mode{mode}_gradz_power_mix_w0_local_mode2": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_mode2",
            f"aw_mode{mode}_gradz_power_mix_w0_local_dx": f"m4d_int_aw_mode{mode}_gradz_power_mix_w0_local_dx",
...
def main():
    ...
    if not rows:
        raise SystemExit("No local-gradient mix_a structure rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-167 Local-Gradient mix_a Structure Audit\n\n")
        ...
        for key in sorted(grouped.keys(), key=lambda x: (x[0], x[1], window_order.get(x[2], 99))):
            ...
            grp = grouped[key]
            target = next((r for r in grp if r["component"] == f"d_aw_mode{mode}_gradz_power_mix_w0_local_gradient_dt"), None)
            f.write(f"## {case_name} mode={mode} window={window}\n\n")
            if target:
                f.write(f"- target_rms_abs_rate: `{fmt(target['rms_abs_rate'])}`\n\n")
            ranked = [r for r in grp if r["component"] != f"d_aw_mode{mode}_gradz_power_mix_w0_local_gradient_dt"]
