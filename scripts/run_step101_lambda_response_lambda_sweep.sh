...
OUTPUT_ROOT="/projects/fluid-engine/out/step101_lambda_response_lambda_sweep_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-101 lambda-response x lambda sweep.

Goal:
  Hold conservative mode-2 seeding fixed, sweep static lambda while response_lambda
  forcing is enabled, then compare brane proxies, projected closure, and ledgers.

Usage:
  ./scripts/run_step101_lambda_response_lambda_sweep.sh [options]
...
run_case() {
  ...
  local cmd=(
    ...
    # Keep controlled deck at its native lambda so ratios remain interpretable.
    --full-arg "modes4d/lambda=${lambda_value}"
  )
...
run_case "lambda_resp_g3_l0p50_t0p60" "0.50" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_resp_g3_l1p00_t0p60" "1.00" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_resp_g3_l2p00_t0p60" "2.00" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_resp_g3_l2p00_t1p20" "2.00" "${LONG_TLIM}" "${LONG_NLIM}"
...
echo ""
echo "Step-101 lambda-response x lambda sweep complete."
