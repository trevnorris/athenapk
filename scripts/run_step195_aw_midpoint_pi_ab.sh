...
OUTPUT_ROOT="/projects/fluid-engine/out/step195_aw_midpoint_pi_ab_local"
...
usage() {
  cat <<'USAGE'
Run Step-195 A_w midpoint-pi update A/B on the corrected quadrature baseline.
USAGE
}
...
run_case() {
  ...
  local cmd=(python3 "${REPO_ROOT}/scripts/harris_scan_matrix.py" --binary "${BINARY}" --workdir "${WORKDIR}" --controlled-input "${CONTROLLED_INPUT}" --full-input "${FULL_INPUT}" --output-dir "${case_dir}" --fail-on-nonfinite-hst --check-history-nonfinite --check-transport-closure --check-em-bulk-ledger --check-topology --check-mechanism)
  for arg in "${COMMON_MESH_ARGS[@]}"; do cmd+=(--athena-arg "${arg}"); done
  for arg in "${CONTROLLED_ARGS[@]}"; do cmd+=(--controlled-arg "${arg}"); done
  for arg in "${FULL_BASE_ARGS[@]}"; do cmd+=(--full-arg "${arg}"); done
  for arg in "$@"; do cmd+=(--full-arg "${arg}"); done
...
run_case "aw_midpoint_pi" "modes4d/aw_update_midpoint_pi=true"
...
printf 'aw_midpoint_pi_ab_md,%s\n' "${OUTPUT_ROOT}_aw_midpoint_pi_ab.md"
