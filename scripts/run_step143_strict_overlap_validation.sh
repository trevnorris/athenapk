...
OUTPUT_ROOT="/projects/fluid-engine/out/step143_strict_overlap_validation_local"
...
usage() {
  cat <<'USAGE'
Run Step-143 strict controlled-path overlap validation.

Goal:
  Verify overlap between:
    - controlled boundedness at production horizon
    - full transfer-floor activation + predictive pass
  by comparing legacy-vs-strict controlled solver path under identical full
  Harris settings.

Usage:
  ./scripts/run_step143_strict_overlap_validation.sh [options]
...
run_case() {
  ...
  for arg in "${CONTROLLED_BOUND_ARGS_BASE[@]}"; do
    cmd+=(--controlled-arg "${arg}")
  done
  cmd+=(--controlled-arg "modes4d/hard_controlled_limit_strict_solver_path=${strict_path}")
...
run_case "harris_legacy_controlled_path" "false"
run_case "harris_strict_controlled_path" "true"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "harris_legacy_controlled_path" \
...
echo ""
echo "Step-143 strict overlap validation complete."
