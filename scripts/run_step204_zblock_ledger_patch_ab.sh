...
OUTPUT_ROOT="/projects/fluid-engine/out/step204_zblock_ledger_patch_local"
...
usage() {
  cat <<'USAGE'
Run Step-204 direct z-block ledger patch A/B.

This uses the corrected quadrature baseline and turns on the diagnostic bulk-energy
z-block correction inside m4d_em_u_bulk while preserving m4d_em_u_bulk_legacy.
USAGE
}
...
CASE_DIR="${OUTPUT_ROOT}/zblock_patch_candidate"
...
if [[ "${RESUME}" == "true" ]] && [[ -f "${EXIT_FILE}" ]]; then
  prior_rc="$(<"${EXIT_FILE}")"
  if [[ "${prior_rc}" == "0" ]]; then
    echo "=== zblock_patch_candidate ===" | tee -a "${LOG_FILE}"
    echo "case_status,zblock_patch_candidate,SKIP,resume_hit" | tee -a "${LOG_FILE}"
  else
    rm -f "${EXIT_FILE}"
  fi
fi
...
if [[ ! -f "${EXIT_FILE}" ]]; then
  ...
  echo "=== zblock_patch_candidate ===" | tee "${LOG_FILE}"
  ...
  echo "${rc}" > "${EXIT_FILE}"
  if [[ ${rc} -eq 0 ]]; then
    echo "case_status,zblock_patch_candidate,PASS" | tee -a "${LOG_FILE}"
  else
    echo "case_status,zblock_patch_candidate,FAIL,rc=${rc}" | tee -a "${LOG_FILE}"
    exit ${rc}
  fi
fi
