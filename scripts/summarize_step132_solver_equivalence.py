#!/usr/bin/env python3
"""Summarize Step-132 controlled solver-equivalence probe outputs."""
...
def main() -> None:
    ...
    for case_dir in case_dirs:
        ...
        row = {
            ...
            "controlled_perturbation_amp": _last_cmdline_value(
                run_log, "problem/harris_4d/perturbation_amp"
            ),
            "error": "",
        }
        rows.append(row)
    ...
    lines: List[str] = []
    lines.append("# Step-132 Controlled Solver-Equivalence Summary")
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "| case | status | controlled psi0 | psi0 reduction vs baseline | ledger reduction vs baseline | hard-limit | n_quadrature (controlled) | resistivity | perturb_amp |"
    )
    lines.append("|---|---|---:|---:|---:|---|---:|---|---:|")
    for row in rows:
        lines.append(
            ...
            f"{row['controlled_perturbation_amp'] or 'default'} |"
        )
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
