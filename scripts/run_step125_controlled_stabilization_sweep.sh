...
OUTPUT_ROOT="/projects/fluid-engine/out/step125_controlled_stabilization_sweep_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-125 controlled-stabilization sweep.

Goal:
  Keep a known transfer-active full configuration (t100 plus-force) while
  sweeping controlled-only stabilization knobs to find a bounded baseline.

Usage:
  ./scripts/run_step125_controlled_stabilization_sweep.sh [options]
...
# Anchor full branch on step124 t100_plus_force (active-transfer PASS).
...
# Baseline: expected to be unbounded at this horizon.
run_case "ctrl_default"

# Controlled branch source clamp.
run_case "ctrl_sources_zero" \
...
# Source clamp + extra controlled damping.
run_case "ctrl_sources_zero_damped" \
  "modes4d/em_source_current_gain=0.0" \
  "modes4d/em_source_timelike_gain=0.0" \
  "modes4d/plasma_momw_source_gain=0.0" \
  "modes4d/plasma_momw_pressure_source_gain=0.0" \
...
# Partial source reduction (less aggressive).
run_case "ctrl_sources_quarter" \
  "modes4d/em_source_current_gain=2.5e-1" \
  "modes4d/em_source_timelike_gain=2.5e-1" \
  "modes4d/plasma_momw_source_gain=2.5e-1" \
  "modes4d/plasma_momw_pressure_source_gain=2.5e-1" \
  "modes4d/plasma_force_source_gain=2.5e-1" \
  "modes4d/plasma_energy_source_gain=2.5e-1" \
  "modes4d/plasma_momw_damping=5.0e-1"
...
echo ""
echo "Step-125 controlled-stabilization sweep complete."
