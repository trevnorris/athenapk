...
OUTPUT_ROOT="/projects/fluid-engine/out/step109_w0_dynamic_mixing_ab_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-109 global w0(t) dynamic-mixing A/B probe.

Goal:
  A/B test the new conservative dynamic w0(t) mode-mixing path
  (off vs on, plus stronger gain) after Step-108 work/leak coupling.

Usage:
  ./scripts/run_step109_w0_dynamic_mixing_ab.sh [options]
...
# 0) reference from Step-108/107 (no w0 response path)
run_case "combined_ref" \
  "modes4d/response_w0_enable=false"

# 1) w0 response enabled (no dynamic mixing)
run_case "w0_response_nomix" \
...
# 2) same as case 1 + dynamic global w0 mixing (gain 1)
run_case "w0_response_mix_g1" \
  "modes4d/response_w0_enable=true" \
  "modes4d/response_w0_drive_gain=3.0" \
  "modes4d/response_w0_drive_from_work_gain=2.0e4" \
  "modes4d/response_w0_drive_from_work_abs=true" \
...
# 3) stronger dynamic global w0 mixing (gain 4)
run_case "w0_response_mix_g4" \
  "modes4d/response_w0_enable=true" \
  "modes4d/response_w0_drive_gain=3.0" \
  "modes4d/response_w0_drive_from_work_gain=2.0e4" \
  "modes4d/response_w0_drive_from_work_abs=true" \
...
echo ""
echo "Step-109 w0 dynamic mixing A/B probe complete."
