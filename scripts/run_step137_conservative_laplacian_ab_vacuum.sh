...
OUTPUT_ROOT="/projects/fluid-engine/out/step137_conservative_laplacian_ab_vacuum_local"
...
usage() {
  cat <<'USAGE'
Run Step-137 conservative-vs-source Laplacian A/B in the fast vacuum harness.

Goal:
  Test whether runaway is specific to non-conservative source-Laplacian update,
  by comparing:
    - source-path Laplacian cases (em_conservative_transport=false)
    - conservative EM transport path (em_conservative_transport=true)
  with Laplacian gain/sign toggles and damping cross-check.

Usage:
  ./scripts/run_step137_conservative_laplacian_ab_vacuum.sh [options]
...
# Source-path Laplacian controls.
run_case "source_lap_baseline" \
...
run_case "source_lap_off" \
  "modes4d/em_conservative_transport=false" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=0.0"

run_case "source_lap_flip" \
  "modes4d/em_conservative_transport=false" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=-1.0"

run_case "source_lap_baseline_damping_on" \
  "modes4d/em_conservative_transport=false" \
  "modes4d/em_damping=0.10" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=1.0"

# Conservative transport-path cross-checks.
run_case "cons_baseline" \
  "modes4d/em_conservative_transport=true" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=1.0"

run_case "cons_lap_flip" \
  "modes4d/em_conservative_transport=true" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=-1.0"

run_case "cons_baseline_damping_on" \
  "modes4d/em_conservative_transport=true" \
  "modes4d/em_damping=0.10" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_laplacian_gain=1.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step135_em_term_isolation.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "source_lap_baseline" \
  --out-csv "${OUTPUT_ROOT}_summary.csv" \
  --out-md "${OUTPUT_ROOT}_summary.md"

echo ""
echo "Step-137 conservative-vs-source Laplacian A/B complete."
