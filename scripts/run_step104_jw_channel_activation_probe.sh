...
OUTPUT_ROOT="/projects/fluid-engine/out/step104_jw_channel_activation_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-104 Jw-channel activation probe.

Goal:
  Explicitly activate transverse current via physical initializer seed
  (problem/harris_4d/drift_momw_scale) and measure whether spillback proxies
  and mixed-sector transfer channels lift off zero.

Usage:
  ./scripts/run_step104_jw_channel_activation_probe.sh [options]
...
# 0) baseline (no transverse-current seed)
run_case "baseline"

# 1) gentle J^w seed
run_case "jw_seed_p002" \
  "problem/harris_4d/drift_momw_scale=2.0e-2"

# 2) stronger J^w seed
run_case "jw_seed_p005" \
  "problem/harris_4d/drift_momw_scale=5.0e-2"

# 3) sign-flipped J^w seed (symmetry check)
run_case "jw_seed_m005" \
  "problem/harris_4d/drift_momw_scale=-5.0e-2"

python3 "${REPO_ROOT}/scripts/summarize_jw_activation.py" \
  --glob "${OUTPUT_ROOT}/*" \
...
echo ""
echo "Step-104 Jw-channel activation probe complete."
