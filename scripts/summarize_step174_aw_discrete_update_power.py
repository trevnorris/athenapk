#!/usr/bin/env python3
"""Compare discrete A_w update-power proxies against diagonal (d_z a_w)^2 drift."""
...
def compute_rows(case_dir, cols, fractions):
    ...
    for mode in (1, 2):
        ...
        series_keys = {
            f"aw_mode{mode}_gradz_power_pi_drive": f"m4d_int_aw_mode{mode}_gradz_power_pi_drive",
            f"aw_mode{mode}_gradz_power_pi_drive_discrete": f"m4d_int_aw_mode{mode}_gradz_power_pi_drive_discrete",
            ...
            f"aw_mode{mode}_gradz_power_da_dt_discrete": f"m4d_int_aw_mode{mode}_gradz_power_da_dt_discrete",
        }
...
def main():
    ...
    if not rows:
        raise SystemExit("No discrete update-power rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-174 A_w Discrete Update-Power Probe\n\n")
