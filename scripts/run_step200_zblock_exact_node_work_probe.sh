...
OUTPUT_ROOT="/projects/fluid-engine/out/step200_zblock_exact_node_work_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-200 exact-node z-block work probe on the corrected quadrature baseline.

Goal:
  Compare exact Cx/Cz energy deltas against exact-node A_w,x / A_w,z linear work
  channels computed at the same quadrature-node level.

Usage:
  ./scripts/run_step200_zblock_exact_node_work_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step200_zblock_exact_node_work_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_zblock_exact_node_work.csv" \
  --out-md "${OUTPUT_ROOT}_zblock_exact_node_work.md"
...
echo "zblock_exact_node_work_md,${OUTPUT_ROOT}_zblock_exact_node_work.md"
