...
OUTPUT_ROOT="/projects/fluid-engine/out/step129_controlled_quadrature_collapse_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-129 controlled quadrature-collapse probe.

Goal:
  Test whether forcing controlled runs to n_quadrature=1 (hard-path proxy for
  Nw=1 limit) materially reduces controlled runaway metrics while preserving
  full-branch transfer activation.

Usage:
  ./scripts/run_step129_controlled_quadrature_collapse.sh [options]
...
# Controlled hard-path proxy: force n_quadrature = 1.
run_case "ctrl_nq1" \
  "modes4d/n_quadrature=1"

# n_quadrature collapse + diffusion off (checks interaction with resistivity path).
run_case "ctrl_nq1_resnone" \
  "modes4d/n_quadrature=1" \
...
python3 "${REPO_ROOT}/scripts/summarize_step129_controlled_quadrature.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "ctrl_baseline" \
  --out-csv "${OUTPUT_ROOT}_quadrature_probe.csv" \
  --out-md "${OUTPUT_ROOT}_quadrature_probe.md"

echo ""
echo "Step-129 controlled quadrature-collapse probe complete."
...
echo "quadrature_probe_md,${OUTPUT_ROOT}_quadrature_probe.md"
