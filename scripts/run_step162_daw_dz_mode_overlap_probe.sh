...
OUTPUT_ROOT="/projects/fluid-engine/out/step162_daw_dz_mode_overlap_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-162 d_z a_w mode-overlap probe on the corrected quadrature baseline.

Goal:
  Decompose the dominant (d_z a_w)^2 channel into mode-diagonal and mode-cross
  pieces to see whether the remaining EM bulk ledger drift is another overlap bug
  or a single-mode bookkeeping defect.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step162_daw_dz_mode_overlap_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step162_daw_dz_mode_overlap.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_daw_dz_mode_overlap.csv" \
  --out-md "${OUTPUT_ROOT}_daw_dz_mode_overlap.md"
...
echo "daw_dz_mode_overlap_md,${OUTPUT_ROOT}_daw_dz_mode_overlap.md"
