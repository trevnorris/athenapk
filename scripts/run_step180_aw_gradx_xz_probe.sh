...
OUTPUT_ROOT="/projects/fluid-engine/out/step180_aw_gradx_xz_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-180 A_w grad-x / x+z discrete update-power probe on the corrected quadrature baseline.

Goal:
  Mirror the A_w,z discrete update-power audit for the A_w,x channel and test whether the combined x+z all-mode discrete update-power closes the surviving EM bulk ledger residual.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step180_aw_gradx_xz_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step180_aw_gradx_xz_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_gradx_xz.csv" \
  --out-md "${OUTPUT_ROOT}_aw_gradx_xz.md"
...
echo "aw_gradx_xz_md,${OUTPUT_ROOT}_aw_gradx_xz.md"
