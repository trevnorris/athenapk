#!/usr/bin/env python3
"""Test fixed x/z combination rules against the surviving EM bulk-ledger residual."""
...
def scale_series(series, factor):
    return [factor * v for v in series]
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        components = {
            "gradz_unit": gradz,
            "gradx_unit": gradx,
            "gradz_plus_gradx_unit": add_series(gradz, gradx),
            "gradz_minus_gradx_unit": sub_series(gradz, gradx),
            "sqrtpi_gradz_unit": scale_series(gradz, sqrt_pi),
            "sqrtpi_gradz_plus_gradx_unit": add_series(scale_series(gradz, sqrt_pi), gradx),
            "sqrtpi_gradz_minus_gradx_unit": sub_series(scale_series(gradz, sqrt_pi), gradx),
        }
        ...
        rows.append({
            ...
            "fit_coeff": 1.0,
            "corrected_rms_abs_rate_fit": 0.0,
            "unit_corrected_rms_abs_rate": 0.0,
            "fit_reduction_factor": math.inf,
            "unit_reduction_factor": math.inf,
        })

        for name, vals in components.items():
            _, rms_abs, _ = finite_stats(vals)
            alpha = fit_single(y, vals)
            corrected_fit = [yy - alpha * vv for yy, vv in zip(y, vals)] if math.isfinite(alpha) else []
            corrected_unit = [yy - vv for yy, vv in zip(y, vals)]
            _, corrected_fit_rms, _ = finite_stats(corrected_fit)
            _, corrected_unit_rms, _ = finite_stats(corrected_unit)
            fit_reduction = res_rms / corrected_fit_rms if corrected_fit_rms > 0 else math.inf
            unit_reduction = res_rms / corrected_unit_rms if corrected_unit_rms > 0 else math.inf
            rows.append({
                "case_name": case_dir.name,
                "candidate": name,
                ...
                "fit_coeff": alpha,
                "corrected_rms_abs_rate_fit": corrected_fit_rms,
                "unit_corrected_rms_abs_rate": corrected_unit_rms,
                "fit_reduction_factor": fit_reduction,
                "unit_reduction_factor": unit_reduction,
            })

    if not rows:
        raise SystemExit("No Step-187 rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-187 Fixed Combination Rule Probe\n\n")
        ...
        for case_name in sorted({r["case_name"] for r in rows}):
            ...
            target = next(r for r in rows if r["case_name"] == case_name and r["candidate"] == "em_bulk_ledger_residual")
            f.write(f"- residual_rms_abs_rate: `{fmt(target['rms_abs_rate'])}`\n\n")
            f.write("| candidate | corr_with_residual | fit_coeff | corrected_rms_fit | corrected_rms_unit | fit_reduction | unit_reduction |\n")
            f.write("|:---|---:|---:|---:|---:|---:|---:|\n")
            ranked = [r for r in rows if r["case_name"] == case_name and r["candidate"] != "em_bulk_ledger_residual"]
            ranked.sort(key=lambda r: r["unit_reduction_factor"], reverse=True)
            for r in ranked:
                f.write(
                    f"| {r['candidate']} | {fmt(r['corr_with_residual'])} | {fmt(r['fit_coeff'])} | "
                    f"{fmt(r['corrected_rms_abs_rate_fit'])} | {fmt(r['unit_corrected_rms_abs_rate'])} | "
                    f"{fmt(r['fit_reduction_factor'])} | {fmt(r['unit_reduction_factor'])} |\n"
                )
            f.write("\n")
