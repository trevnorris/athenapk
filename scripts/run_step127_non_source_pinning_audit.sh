...
OUTPUT_ROOT="/projects/fluid-engine/out/step127_non_source_pinning_audit_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-127 non-source controlled pinning audit.

Goal:
  Keep the full branch fixed in a known transfer-active configuration and sweep
  controlled-only non-source knobs (constraint/transport/diffusion) to identify
  which non-source channels dominate controlled runaway.

Usage:
  ./scripts/run_step127_non_source_pinning_audit.sh [options]
...
Options:
  ...
  --nlim INT               Runtime nlim (default: 100000)
  -h, --help               Show help
USAGE
}
...
# Full branch anchored to transfer-active Step-124 t100_plus_force setup.
FULL_PLUS_FORCE_ARGS=(
  "modes4d/lambda=1.0"
...
run_case() {
  ...
  for arg in "${CONTROLLED_BASE_SAFE_ARGS[@]}"; do
    cmd+=(--controlled-arg "${arg}")
  done
  for arg in "${FULL_PLUS_FORCE_ARGS[@]}"; do
    cmd+=(--full-arg "${arg}")
  done
  for arg in "$@"; do
    cmd+=(--controlled-arg "${arg}")
...
# Baseline reference at t100.
run_case "ctrl_baseline"

# Constraint-coupling sweeps.
run_case "ctrl_rho_divj_off" \
  "modes4d/plasma_rho_divj_gain=0.0"

run_case "ctrl_rho_divj_hi" \
  "modes4d/plasma_rho_divj_gain=5.0"

# Pressure-transport (Rusanov) sweeps.
run_case "ctrl_rusanov_off" \
  "modes4d/plasma_pressure_rusanov_gain=0.0"

run_case "ctrl_rusanov_hi" \
  "modes4d/plasma_pressure_rusanov_gain=3.0"

# Tighter pressure transport caps.
run_case "ctrl_transport_caps_tight" \
  "modes4d/plasma_pressure_signal_speed_cap=2.0" \
  "modes4d/plasma_pressure_flux_relative_cap=5.0"

# Diffusion-family sweep.
run_case "ctrl_resistivity_none" \
  "diffusion/resistivity=none"

run_case "ctrl_resistivity_none_rho_divj_hi" \
  "diffusion/resistivity=none" \
  "modes4d/plasma_rho_divj_gain=5.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step127_non_source_pinning.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --controlled-bounds-csv "${OUTPUT_ROOT}_controlled_bounds.csv" \
  --baseline-case "ctrl_baseline" \
  --out-csv "${OUTPUT_ROOT}_non_source_pinning.csv" \
  --out-md "${OUTPUT_ROOT}_non_source_pinning.md"

echo ""
echo "Step-127 non-source controlled pinning audit complete."
...
echo "non_source_pinning_md,${OUTPUT_ROOT}_non_source_pinning.md"
