#!/usr/bin/env python3
"""Summarize Step-218 proxy calibration on the post-patch branch."""
...
def main():
    ...
    if missing:
        raise SystemExit(f'Missing required Step-218 cases: {missing}')
    ...
    ordered_crossings = sorted(
        ((name, t) for name, t in forced_cross.items() if math.isfinite(t)),
    ...
    with out_md.open('w', encoding='utf-8') as f:
        f.write('# Step-218 Post-Patch Proxy Calibration\n\n')
        f.write('This A/B compares the clean reference against the strongest Step-217 environment case to see which internal proxy moves first as retained release appears.\n\n')
        ...
        f.write('\n## Reference Envelopes\n\n')
        for proxy_name, threshold in ref_envelopes.items():
            f.write(f'- `{proxy_name}` reference envelope after gate-on: `{fmt(threshold)}`\n')
        f.write('\n## Forced-Case Crossing Times\n\n')
        for proxy_name, t_cross in ordered_crossings:
            f.write(f'- `{proxy_name}` first exceeds its reference envelope at `t = {fmt(t_cross)}`\n')
        ...
        for proxy_name in ('Lz_mixed_cz2', 'Ltot_mixed_c2', 'Bz_zblock_correction', 'Qz_partition', 'Chi_sigma_star'):
            f.write(f'- `{proxy_name}(t_R)` = `{fmt(snapshot[proxy_name])}`\n')
        f.write('\n## Interpretation Guide\n\n')
        f.write('- `Lz_mixed_cz2`: dominant z-channel support loading\n')
        f.write('- `Ltot_mixed_c2`: total mixed support loading\n')
        f.write('- `Bz_zblock_correction`: current best geometry/closure-side proxy\n')
        f.write('- `Qz_partition = Bz/(Lz+eps)`: z-block partition proxy\n')
        f.write(f'- `Chi_sigma_star = Bz/{fmt(args.sigma_star)}`: provisional 2PN bridge scale, heuristic only\n')
