...
OUTPUT_ROOT="/projects/fluid-engine/out/step111_lambda_pure_mixing_conservation_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-111 pure lambda-dynamic-mixing conservation A/B probe.

Goal:
  Isolate the global lambda(t) dynamic-mixing operator effect by disabling
  response drives and comparing no-mix vs mix gains with identical
  lambda_dot_init.

Usage:
  ./scripts/run_step111_lambda_pure_mixing_conservation.sh [options]
...
COMMON_FULL_ARGS=(
  ...
  "modes4d/response_lambda_init_fraction=0.0"
  "modes4d/response_lambda_dot_init=2.0e-1"
  "modes4d/response_lambda_drive_gain=0.0"
  "modes4d/response_lambda_drive_from_bridge_gain=0.0"
  "modes4d/response_lambda_drive_from_work_gain=0.0"
  "modes4d/response_lambda_drive_from_leak_gain=0.0"
  "modes4d/response_lambda_stiffness=0.0"
  "modes4d/response_lambda_damping=0.0"
  "modes4d/response_lambda_apply_mass=false"
)
...
run_case "lambda_pure_nomix" \
  "modes4d/response_lambda_dynamic_mixing_enable=false"

run_case "lambda_pure_mix_g1" \
  "modes4d/response_lambda_dynamic_mixing_enable=true" \
  "modes4d/response_lambda_dynamic_mixing_gain=1.0"

run_case "lambda_pure_mix_g4" \
  "modes4d/response_lambda_dynamic_mixing_enable=true" \
  "modes4d/response_lambda_dynamic_mixing_gain=4.0"
...
python3 "${REPO_ROOT}/scripts/summarize_lambda_mixing_conservation.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_mixing_conservation.csv" \
  --out-md "${OUTPUT_ROOT}_mixing_conservation.md"

echo ""
echo "Step-111 pure lambda dynamic-mixing conservation probe complete."
