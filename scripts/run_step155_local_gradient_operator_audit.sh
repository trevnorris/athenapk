...
OUTPUT_ROOT="/projects/fluid-engine/out/step155_local_gradient_operator_audit_local"
...
usage() {
  cat <<'USAGE'
Run Step-155 local-gradient operator audit.

Goal:
  Follow Step-154 by auditing the `w0_local_gradient` operator itself:
  local-warp mode content, gradient-direction content, and the resulting
  local-gradient mixing rate on the focused Harris baseline.

Case matrix:
  - baseline_local_grad_operator
  - local_mode1_only
  - local_mode2_only

Usage:
  ./scripts/run_step155_local_gradient_operator_audit.sh [options]
...
run_case "baseline_local_grad_operator"
run_case "local_mode1_only" "modes4d/response_w0_local_mode2_amp=0.0"
run_case "local_mode2_only" "modes4d/response_w0_local_mode1_amp=0.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_local_grad_operator" \
...
python3 "${REPO_ROOT}/scripts/summarize_step155_local_gradient_operator.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_local_grad_operator" \
  --out-csv "${OUTPUT_ROOT}_local_gradient_operator.csv" \
  --out-md "${OUTPUT_ROOT}_local_gradient_operator.md"

echo ""
echo "Step-155 local-gradient operator audit complete."
...
echo "local_gradient_operator_md,${OUTPUT_ROOT}_local_gradient_operator.md"
