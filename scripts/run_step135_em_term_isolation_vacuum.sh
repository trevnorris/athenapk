...
OUTPUT_ROOT="/projects/fluid-engine/out/step135_em_term_isolation_vacuum_local"
...
usage() {
  cat <<'USAGE'
Run Step-135 EM-term isolation in the fast vacuum harness.

Goal:
  Isolate which EM RHS term families correlate with the Step-134 runaway:
    - mass term
    - damping path
    - gauge path
  while keeping the same short, controlled Nw=1 vacuum-style setup.

Usage:
  ./scripts/run_step135_em_term_isolation_vacuum.sh [options]
...
# Baseline mirrors Step-134 controlled vacuum setup.
run_case "term_baseline" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_mass_gain=1.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_gauge_gain=0.0"

# Remove mass-like RHS piece.
run_case "term_mass_off" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_mass_gain=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_gauge_gain=0.0"

# Turn on damping path and isolate its contribution.
run_case "term_damping_on" \
  "modes4d/em_damping=0.10" \
  "modes4d/em_source_mass_gain=1.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_gauge_gain=0.0"

run_case "term_damping_on_gain0" \
  "modes4d/em_damping=0.10" \
  "modes4d/em_source_mass_gain=1.0" \
  "modes4d/em_source_damping_gain=0.0" \
  "modes4d/em_source_gauge_gain=0.0"

# Turn on gauge residual source path.
run_case "term_gauge_on" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_mass_gain=1.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_gauge_gain=1.0"

run_case "term_mass_off_gauge_on" \
  "modes4d/em_damping=0.0" \
  "modes4d/em_source_mass_gain=0.0" \
  "modes4d/em_source_damping_gain=1.0" \
  "modes4d/em_source_gauge_gain=1.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step135_em_term_isolation.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "term_baseline" \
  --out-csv "${OUTPUT_ROOT}_summary.csv" \
  --out-md "${OUTPUT_ROOT}_summary.md"

echo ""
echo "Step-135 EM-term isolation (vacuum) complete."
