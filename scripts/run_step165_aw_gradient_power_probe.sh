...
OUTPUT_ROOT="/projects/fluid-engine/out/step165_aw_gradient_power_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-165 A_w gradient-power probe on the corrected quadrature baseline.

Goal:
  Compare the dominant per-mode (d_z a_w)^2 energy drifts against an
  integration-by-parts gradient-power proxy built from the A_w update pieces.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step165_aw_gradient_power_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step165_aw_gradient_power.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_gradient_power.csv" \
  --out-md "${OUTPUT_ROOT}_aw_gradient_power.md"
...
echo "aw_gradient_power_md,${OUTPUT_ROOT}_aw_gradient_power.md"
