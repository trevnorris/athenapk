...
OUTPUT_ROOT="/projects/fluid-engine/out/step150_em_pi_transport_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-150 EM pi transport probe.

Goal:
  Follow Step-149 by testing whether the remaining transport/ledger blocker
  tracks the explicit EM pi seed family (`piw_mode1`, `piy_mode1`,
  `a0_mode2`, `piw_mode2`) or survives when those seeds are removed.

Case matrix:
  - baseline_pi_transport
  - no_piw_mode1
  - no_piy_mode1
  - no_mode1_pi
  - no_a0_mode2
...
Usage:
  ./scripts/run_step150_em_pi_transport_probe.sh [options]
...
Options:
  ...
  --resume BOOL            Skip already PASS cases in output dir (default: true)
  -h, --help               Show help
USAGE
}
...
run_case "baseline_pi_transport"
run_case "no_piw_mode1" "problem/harris_4d/piw_mode1_amp=0.0"
run_case "no_piy_mode1" "problem/harris_4d/piy_mode1_amp=0.0"
run_case "no_mode1_pi" \
  "problem/harris_4d/piw_mode1_amp=0.0" \
  "problem/harris_4d/piy_mode1_amp=0.0"
run_case "no_a0_mode2" "problem/harris_4d/a0_mode2_amp=0.0"
run_case "no_piw_mode2" "problem/harris_4d/piw_mode2_amp=0.0"
run_case "no_all_pi_seeds" \
  "problem/harris_4d/piw_mode1_amp=0.0" \
  "problem/harris_4d/piy_mode1_amp=0.0" \
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_pi_transport" \
...
python3 "${REPO_ROOT}/scripts/summarize_step150_em_pi_transport_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_pi_transport" \
  --out-csv "${OUTPUT_ROOT}_em_pi_transport.csv" \
  --out-md "${OUTPUT_ROOT}_em_pi_transport.md"

echo ""
echo "Step-150 EM pi transport probe complete."
...
echo "em_pi_transport_md,${OUTPUT_ROOT}_em_pi_transport.md"
