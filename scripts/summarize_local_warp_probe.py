#!/usr/bin/env python3
"""Summarize Step-112 local warp probe runs."""
...
def main():
        ...
        rows.append(
            {
                ...
                "local_amp1": parse_arg_from_log(
                    log_path, "modes4d/response_w0_local_mode1_amp"
                ),
                "local_kx1": parse_arg_from_log(
                    log_path, "modes4d/response_w0_local_mode1_kx"
                ),
                "local_kz1": parse_arg_from_log(
                    log_path, "modes4d/response_w0_local_mode1_kz"
                ),
                "local_omega1": parse_arg_from_log(
                    log_path, "modes4d/response_w0_local_mode1_omega"
                ),
                "local_amp2": parse_arg_from_log(
                    log_path, "modes4d/response_w0_local_mode2_amp"
                ),
                "local_omega2": parse_arg_from_log(
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Local Warp Probe Summary\n\n")
        f.write(f"Parsed `{len(rows)}` cases from glob `{args.glob}`.\n\n")
        if not rows:
            f.write("No valid case outputs found.\n")
        else:
            f.write(
                "| case | mix_gain | local amp1/kx1/kz1/omega1 | local amp2/omega2 | local|w0| | local|w0_dot| | mix_a_abs | mix_pi_abs | psi0 ratio | psi_w0-psi_proj | jw_ew | s_leak_abs | projected/local/transport/ledger |\n"
            )
            f.write(
                "|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|\n"
            )
                ...
                f.write(
                    ...
                    + " | ".join(
                        [
                            r["case_name"],
                            fmt(r["mix_gain"]),
                            f"{fmt(r['local_amp1'])}/{fmt(r['local_kx1'])}/{fmt(r['local_kz1'])}/{fmt(r['local_omega1'])}",
                            f"{fmt(r['local_amp2'])}/{fmt(r['local_omega2'])}",
                            fmt(r["full_final_response_w0_local_abs"]),
                            fmt(r["full_final_response_w0_local_dot_abs"]),
