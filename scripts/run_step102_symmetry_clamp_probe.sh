...
OUTPUT_ROOT="/projects/fluid-engine/out/step102_symmetry_clamp_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-102 symmetry-clamp probe.

Goal:
  Test whether exact-zero brane proxies are symmetry/projection clamped by
  comparing centered observer vs small +/- projection-center offsets.

Usage:
  ./scripts/run_step102_symmetry_clamp_probe.sh [options]
...
# off-center observer (+w)
run_case "observer_offset_p010" \
  "modes4d/response_w0_enable=true" \
  "modes4d/response_w0_init=1.0e-1" \
  ...
  "modes4d/response_w0_projection_max_abs=2.0e-1"

# off-center observer (-w)
run_case "observer_offset_m010" \
  "modes4d/response_w0_enable=true" \
  "modes4d/response_w0_init=-1.0e-1" \
  "modes4d/response_w0_drive_gain=0.0" \
  "modes4d/response_w0_drive_from_bridge_gain=0.0" \
...
echo ""
echo "Step-102 symmetry clamp probe complete."
