...
OUTPUT_ROOT="/projects/fluid-engine/out/step130_hard_controlled_limit_probe_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-130 hard controlled-limit probe.

Goal:
  A/B test the new hard controlled-limit clamp for Nw=1 runs:
    - suppress mixed-sector channels (Aw/Ew/Ca/Jw couplings) by construction
    - keep full branch unchanged and transfer-active

Usage:
  ./scripts/run_step130_hard_controlled_limit_probe.sh [options]
...
run_case "ctrl_hard_limit" \
  "modes4d/hard_controlled_limit_enable=true"

run_case "ctrl_hard_limit_resnone" \
  "modes4d/hard_controlled_limit_enable=true" \
...
python3 "${REPO_ROOT}/scripts/summarize_step129_controlled_quadrature.py" \
  ...
  --out-csv "${OUTPUT_ROOT}_hard_limit_probe.csv" \
  --out-md "${OUTPUT_ROOT}_hard_limit_probe.md"

echo ""
echo "Step-130 hard controlled-limit probe complete."
...
echo "hard_limit_probe_md,${OUTPUT_ROOT}_hard_limit_probe.md"
