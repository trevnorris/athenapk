#!/usr/bin/env python3
"""Summarize the sub-amp0025 tau-pulse threshold sweep."""
...
CASE_META = {
    "patched_static_pulse_ref": {"t_off": None},
    "patched_tau_pulse_amp0005": {"t_off": 0.65},
    "patched_tau_pulse_amp0010": {"t_off": 0.65},
    "patched_tau_pulse_amp0015": {"t_off": 0.65},
    "patched_tau_pulse_amp0020": {"t_off": 0.65},
}
...
def main():
    ...
    if not rows:
        raise SystemExit("No Step-216 rows computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-216 Sub-amp0025 Tau-Pulse Threshold Sweep\n\n")
        f.write("| case | psi0 ratio | final gap | post-force max gap | post-force rms gap | transport | top transport | ledger |\n")
        f.write("|:---|---:|---:|---:|---:|:---|:---|:---|\n")
        for r in rows:
            f.write(
                f"| {r['case_name']} | {fmt(r['psi0_ratio_full_over_controlled'])} | {fmt(r['final_gap'])} | "
                f"{fmt(r['post_force_max_abs_gap'])} | {fmt(r['post_force_rms_gap'])} | {r['transport_status']} | "
