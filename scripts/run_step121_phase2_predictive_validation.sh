...
OUTPUT_ROOT="/projects/fluid-engine/out/step121_phase2_predictive_validation_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-121 Phase-2 predictive-validation campaign.

Goal:
  Validate that active-transfer channels (JwEw/S_leak/EMF) remain active and
  predictive against psi proxies in the Step-120 active regime.

Usage:
  ./scripts/run_step121_phase2_predictive_validation.sh [options]
...
# Anchor on Step-120 active case.
COMMON_FULL_ARGS=(
  "modes4d/lambda=1.0"
...
# A) Step-120 active reference.
run_case "active_ref_fixed"

# B) Active regime with comoving observer window.
run_case "active_comoving" \
  "modes4d/response_w0_projection_enable=true"

# C) Active regime with slightly higher forcing.
...
echo ""
echo "Step-121 phase-2 predictive validation complete."
