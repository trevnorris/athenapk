...
OUTPUT_ROOT="/projects/fluid-engine/out/step216_postpatch_tau_pulse_threshold_local"
...
usage() {
  cat <<'USAGE'
Run Step-216 sub-amp0025 tau-pulse threshold sweep on the patched branch.
USAGE
}
...
FULL_BASE_ARGS=(
  ...
  "modes4d/response_w0_local_gate_t_on=${PULSE_T_ON}"
  "modes4d/response_w0_local_gate_t_off=${PULSE_T_OFF}"
...
run_case "patched_static_pulse_ref" "0.0" "0.0"
run_case "patched_tau_pulse_amp0005" "6.25e-5" "3.75e-5"
run_case "patched_tau_pulse_amp0010" "1.25e-4" "7.5e-5"
run_case "patched_tau_pulse_amp0015" "1.875e-4" "1.125e-4"
run_case "patched_tau_pulse_amp0020" "2.5e-4" "1.5e-4"
...
python3 "${REPO_ROOT}/scripts/summarize_step216_postpatch_tau_pulse_threshold.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --summary-csv "${OUTPUT_ROOT}_summary.csv" \
  --phase2-csv "${OUTPUT_ROOT}_phase2.csv" \
  --out-csv "${OUTPUT_ROOT}_tau_pulse_threshold.csv" \
  --out-md "${OUTPUT_ROOT}_tau_pulse_threshold.md"
