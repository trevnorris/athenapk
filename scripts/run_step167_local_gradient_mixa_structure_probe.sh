...
OUTPUT_ROOT="/projects/fluid-engine/out/step167_local_gradient_mixa_structure_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-167 local-gradient mix_a structure probe on the corrected quadrature baseline.

Goal:
  Split the dominant w0_local_gradient mix_a gradient-power channel by
  local source mode (mode1/mode2) and direction (dx/dz) for modes 1 and 2.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step167_local_gradient_mixa_structure_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step167_local_gradient_mixa_structure.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_local_gradient_mixa_structure.csv" \
  --out-md "${OUTPUT_ROOT}_local_gradient_mixa_structure.md"
...
echo "local_gradient_mixa_structure_md,${OUTPUT_ROOT}_local_gradient_mixa_structure.md"
