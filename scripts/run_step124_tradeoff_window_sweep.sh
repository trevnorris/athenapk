...
OUTPUT_ROOT="/projects/fluid-engine/out/step124_tradeoff_window_sweep_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-124 tradeoff window sweep.

Goal:
  Find an intermediate horizon where controlled branch stays bounded while
  transfer-floor channels (JwEw/S_leak) remain active.

Usage:
  ./scripts/run_step124_tradeoff_window_sweep.sh [options]
...
run_case() {
  ...
  for arg in "${CONTROLLED_SAFE_ARGS[@]}"; do
...
# Intermediate horizon sweep between step123 (0.35) and step122 (1.50).
run_case "t070_ref_fixed" 0.70 70000
run_case "t070_plus_force" 0.70 70000 \
...
run_case "t100_ref_fixed" 1.00 100000
run_case "t100_plus_force" 1.00 100000 \
  "modes4d/em_source_current_gain=14.0" \
  "modes4d/em_source_timelike_gain=14.0" \
  "modes4d/plasma_momw_source_gain=14.0" \
  "modes4d/plasma_momw_pressure_source_gain=14.0" \
...
run_case "t120_ref_fixed" 1.20 120000
run_case "t120_plus_force" 1.20 120000 \
  "modes4d/em_source_current_gain=14.0" \
  "modes4d/em_source_timelike_gain=14.0" \
  "modes4d/plasma_momw_source_gain=14.0" \
  "modes4d/plasma_momw_pressure_source_gain=14.0" \
...
echo ""
echo "Step-124 tradeoff window sweep complete."
