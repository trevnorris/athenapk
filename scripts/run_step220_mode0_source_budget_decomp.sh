...
OUTPUT_ROOT="/projects/fluid-engine/out/step220_mode0_source_budget_decomp_local"
GATE_T_ON="3.0e-1"
GATE_T_OFF="9.0e-1"
EPS="1.0e-30"
...
usage() {
  cat <<'USAGE'
Run Step-220 mode-0 source-budget decomposition on existing Step-218 outputs.
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step220_mode0_source_budget_decomp.py" \
  --glob "${SOURCE_ROOT}/*" \
