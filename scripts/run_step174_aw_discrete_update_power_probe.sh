...
OUTPUT_ROOT="/projects/fluid-engine/out/step174_aw_discrete_update_power_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-174 A_w discrete update-power probe on the corrected quadrature baseline.

Goal:
  Compare the surviving per-mode (d_z a_w)^2 drift against discrete gradient-power
  diagnostics for the full A_w update path: pi_drive and total da_dt.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step174_aw_discrete_update_power_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step174_aw_discrete_update_power.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_discrete_update_power.csv" \
  --out-md "${OUTPUT_ROOT}_aw_discrete_update_power.md"
...
echo "aw_discrete_update_power_md,${OUTPUT_ROOT}_aw_discrete_update_power.md"
