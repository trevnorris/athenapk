...
OUTPUT_ROOT="/projects/fluid-engine/out/step132_controlled_solver_equivalence_local"
...
usage() {
  cat <<'USAGE'
Run Step-132 controlled solver-equivalence probe.

Goal:
  Distinguish transform-only error from solver-path/model-path effects by
  testing a controlled matrix with progressively stronger "inert" clamps.

Usage:
  ./scripts/run_step132_controlled_solver_equivalence_probe.sh [options]
...
run_case "ctrl_hard_limit" \
  "modes4d/hard_controlled_limit_enable=true"
run_case "ctrl_hard_limit_nq1" \
  "modes4d/hard_controlled_limit_enable=true" \
  "modes4d/n_quadrature=1"
run_case "ctrl_hard_limit_nq1_resnone" \
  "modes4d/hard_controlled_limit_enable=true" \
  "modes4d/n_quadrature=1" \
  "diffusion/resistivity=none"
run_case "ctrl_hard_inert" \
  "${INERT_ARGS[@]}"
run_case "ctrl_hard_inert_noperturb" \
  "${INERT_ARGS[@]}" \
  "problem/harris_4d/perturbation_amp=0.0"
...
echo ""
echo "Step-132 controlled solver-equivalence probe complete."
