...
OUTPUT_ROOT="/projects/fluid-engine/out/step221_mode0_dynamic_mix_onset_local"
GATE_T_ON="3.0e-1"
GATE_T_OFF="9.0e-1"

usage() {
  cat <<'USAGE'
Run Step-221 mode-0 dynamic-mix onset decomposition on existing Step-218 outputs.
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step221_mode0_dynamic_mix_onset.py" \
  --glob "${SOURCE_ROOT}/*" \
