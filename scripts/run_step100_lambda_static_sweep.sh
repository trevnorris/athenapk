...
OUTPUT_ROOT="/projects/fluid-engine/out/step100_lambda_static_sweep_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-100 static-lambda conservative sweep.

This script scans static modes4d/lambda values while forcing all dynamic
response channels off, then writes summary/decomposition markdown reports.

Usage:
  ./scripts/run_step100_lambda_static_sweep.sh [options]
...
Options:
  ...
  --output-root DIR        Output root for all case directories
...
run_case() {
  ...
  local cmd=(
    ...
    --athena-arg "modes4d/lambda=${lambda_value}"
  )
  ...
  echo "=== ${case_name} (lambda=${lambda_value}, tlim=${tlim}, nlim=${nlim}) ===" | tee "${log_file}"
...
# Short screening sweep.
run_case "lambda_0p50_t0p60" "0.50" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_0p75_t0p60" "0.75" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_1p00_t0p60" "1.00" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_1p25_t0p60" "1.25" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_1p50_t0p60" "1.50" "${SHORT_TLIM}" "${SHORT_NLIM}"
run_case "lambda_2p00_t0p60" "2.00" "${SHORT_TLIM}" "${SHORT_NLIM}"

# Long-horizon anchors.
run_case "lambda_1p00_t1p20" "1.00" "${LONG_TLIM}" "${LONG_NLIM}"
run_case "lambda_2p00_t1p20" "2.00" "${LONG_TLIM}" "${LONG_NLIM}"

python3 "${REPO_ROOT}/scripts/summarize_lambda_static_sweep.py" \
  --glob "${OUTPUT_ROOT}/*" \
...
echo ""
echo "Step-100 static lambda sweep complete."
