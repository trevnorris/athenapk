...
OUTPUT_ROOT="/projects/fluid-engine/out/step156_local_gradient_norm_ab_local"
...
usage() {
  cat <<'USAGE'
Run Step-156 local-gradient norm A/B.

Goal:
  Test whether the two-mode cross term inside the local-gradient norm is the
  source of the remaining transport defect.
...
Usage:
  ./scripts/run_step156_local_gradient_norm_ab.sh [options]
USAGE
}
...
run_case "legacy_combined" "modes4d/response_w0_local_gradient_norm_form=combined"
run_case "quadrature_no_cross" "modes4d/response_w0_local_gradient_norm_form=quadrature"
run_case "l1_modes_sum" "modes4d/response_w0_local_gradient_norm_form=l1_modes"
...
python3 "${REPO_ROOT}/scripts/summarize_step156_local_gradient_norm.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "legacy_combined" \
  --out-csv "${OUTPUT_ROOT}_local_gradient_norm.csv" \
  --out-md "${OUTPUT_ROOT}_local_gradient_norm.md"

echo ""
echo "Step-156 local-gradient norm A/B complete."
...
echo "local_gradient_norm_md,${OUTPUT_ROOT}_local_gradient_norm.md"
