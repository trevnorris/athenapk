#!/usr/bin/env python3
"""Summarize directional decomposition of m4d_mixed_c2."""
...
def compute_rows(case_dir, cols, fractions):
    ...
    t_r, mixed_c2_rate = derivative_series(times, cols["m4d_mixed_c2"])
    if not t_r or len(bulk_rate) != len(mixed_c2_rate):
        return []

    component_rates = {
        "d_mixed_cx2_dt": derivative_series(times, cols["m4d_mixed_cx2"])[1],
        "d_mixed_cy2_dt": derivative_series(times, cols["m4d_mixed_cy2"])[1],
        "d_mixed_cz2_dt": derivative_series(times, cols["m4d_mixed_cz2"])[1],
    }
    if any(len(v) != len(mixed_c2_rate) for v in component_rates.values()):
        return []
    ...
    for idx in range(len(fractions) - 1):
        ...
        mixed_window = select_window(t_r, mixed_c2_rate, t_lo, t_hi, include_right)
        bulk_max, bulk_rms, _ = finite_stats(bulk_window)
        mixed_max, mixed_rms, mixed_final = finite_stats(mixed_window)
        if not mixed_window:
            continue
        ...
        rows.append(
            {
                ...
                "window": window,
                "component": "d_mixed_c2_dt",
                "max_abs_rate": mixed_max,
                "rms_abs_rate": mixed_rms,
                "final_rate": mixed_final,
                "share_of_mixed_c2_rms": 1.0,
                "share_of_bulk_rms": mixed_rms / bulk_rms if bulk_rms > 0.0 else math.nan,
                "corr_with_mixed_c2": 1.0,
                "corr_with_bulk": pearson(mixed_window, bulk_window),
                "mixed_c2_rms_abs_rate": mixed_rms,
            ...
            rows.append(
                {
                    ...
                    "share_of_mixed_c2_rms": rms_abs / mixed_rms if mixed_rms > 0.0 else math.nan,
                    "share_of_bulk_rms": rms_abs / bulk_rms if bulk_rms > 0.0 else math.nan,
                    "corr_with_mixed_c2": pearson(window_vals, mixed_window),
...
def main():
    ...
    for d in dirs:
        ...
        required = ["time", "m4d_em_u_bulk", "m4d_mixed_c2", "m4d_mixed_cx2", "m4d_mixed_cy2", "m4d_mixed_cz2"]
        if not all(col in cols for col in required):
            continue
        rows.extend(compute_rows(d, cols, fractions))

    if not rows:
        raise SystemExit("No directional mixed-C rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-160 Mixed C Direction Audit\n\n")
        ...
        for (case_name, window) in sorted(grouped.keys(), key=lambda x: (x[0], window_order.get(x[1], 99))):
            ...
            mixed = next((r for r in grp if r["component"] == "d_mixed_c2_dt"), None)
            f.write(f"## {case_name} window={window}\n\n")
            if mixed is not None:
                f.write(f"- mixed_c2_rms_abs_rate: `{fmt(mixed['rms_abs_rate'])}`\n")
                f.write(f"- bulk_rms_abs_rate: `{fmt(mixed['bulk_rms_abs_rate'])}`\n\n")
            ranked = [r for r in grp if r["component"] != "d_mixed_c2_dt"]
            ranked.sort(key=lambda r: abs(r["share_of_mixed_c2_rms"]) if math.isfinite(r["share_of_mixed_c2_rms"]) else -1.0, reverse=True)
            f.write("| component | rms_abs_rate | share_of_mixed_c2_rms | share_of_bulk_rms | corr_with_mixed_c2 | corr_with_bulk | max_abs_rate | final_rate |\n")
            f.write("|:---|---:|---:|---:|---:|---:|---:|---:|\n")
            for row in ranked:
                f.write(
                    f"| {row['component']} | {fmt(row['rms_abs_rate'])} | {fmt(row['share_of_mixed_c2_rms'])} | {fmt(row['share_of_bulk_rms'])} | {fmt(row['corr_with_mixed_c2'])} | {fmt(row['corr_with_bulk'])} | {fmt(row['max_abs_rate'])} | {fmt(row['final_rate'])} |\n"
                )
            f.write("\n")
