...
OUTPUT_ROOT="/projects/fluid-engine/out/step157_quadrature_ledger_decomposition_local"
...
usage() {
  cat <<'USAGE'
Run Step-157 quadrature-baseline ledger decomposition.

Goal:
  Re-run the Step-148 ledger family decomposition on top of the corrected
  `quadrature_no_cross` local-gradient norm so we can isolate the remaining
  ledger defect after transport is fixed.
...
Usage:
  ./scripts/run_step157_quadrature_ledger_decomposition.sh [options]
USAGE
}
...
echo ""
echo "Step-157 quadrature ledger decomposition complete."
