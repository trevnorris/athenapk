...
OUTPUT_ROOT="/projects/fluid-engine/out/step188_aw_exact_xz_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-188 exact A_w x+z discrete energy-increment probe on the corrected quadrature baseline.

Goal:
  Compare exact discrete x+z A_w energy increments against the surviving EM bulk
  ledger residual on the baseline_quadrature case.

Usage:
  ./scripts/run_step188_aw_exact_xz_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step188_aw_exact_xz_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_exact_xz.csv" \
  --out-md "${OUTPUT_ROOT}_aw_exact_xz.md"
...
echo "aw_exact_xz_md,${OUTPUT_ROOT}_aw_exact_xz.md"
