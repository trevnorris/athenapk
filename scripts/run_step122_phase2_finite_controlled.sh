...
OUTPUT_ROOT="/projects/fluid-engine/out/step122_phase2_finite_controlled_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-122 Phase-2 predictive validation with finite controlled branch.

Goal:
  Re-run Step-121 active cases with a conservative horizon and explicit
  controlled-mode response clamp so full/controlled comparisons stay finite.

Usage:
  ./scripts/run_step122_phase2_finite_controlled.sh [options]
...
Options:
  ...
  --tlim FLOAT             Run tlim (default: 1.50)
  --nlim INT               Run nlim (default: 150000)
  -h, --help               Show help
USAGE
}
...
# Keep full branch anchored on the Step-120 active forcing.
COMMON_FULL_ARGS=(
  "modes4d/lambda=1.0"
...
run_case "active_ref_fixed"

run_case "active_comoving" \
  "modes4d/response_w0_projection_enable=true"

run_case "active_plus_force" \
...
echo ""
echo "Step-122 phase-2 finite-controlled validation complete."
