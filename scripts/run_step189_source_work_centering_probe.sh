...
OUTPUT_ROOT="/projects/fluid-engine/out/step189_source_work_centering_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-189 source-work time-centering probe on the corrected quadrature baseline.

Goal:
  Compare EM bulk ledger closure under alternative time centerings of:
  - ja·ea
  - jw·ew
...
Usage:
  ./scripts/run_step189_source_work_centering_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step189_source_work_centering.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_source_work_centering.csv" \
  --out-md "${OUTPUT_ROOT}_source_work_centering.md"
...
echo "source_work_centering_md,${OUTPUT_ROOT}_source_work_centering.md"
