#!/usr/bin/env python3
"""Summarize mode-diagonal vs mode-cross decomposition of d_z a_w energy."""
...
def compute_rows(case_dir, cols, fractions):
    times = cols["time"]
    t_r, target = derivative_series(times, cols["m4d_mixed_cz_daw_dz2"])
    _, bulk = derivative_series(times, cols["m4d_em_u_bulk"])
    if not t_r or len(target) != len(bulk):
        return []

    component_rates = {
        "d_mixed_cz_daw_dz2_mode_diag_dt": derivative_series(
            times, cols["m4d_mixed_cz_daw_dz2_mode_diag"]
        )[1],
        "d_mixed_cz_daw_dz2_mode_cross_dt": derivative_series(
            times, cols["m4d_mixed_cz_daw_dz2_mode_cross"]
        )[1],
    }
        ...
        rows.append(
            {
                ...
                "window": window,
                "component": "d_mixed_cz_daw_dz2_dt",
                ...
                "target_rms_abs_rate": target_rms,
...
def main():
    ...
    if not rows:
        raise SystemExit("No d_z a_w overlap rows were computed.")
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Step-162 d_z a_w Mode-Overlap Audit\n\n")
        ...
        for (case_name, window) in sorted(grouped.keys(), key=lambda x: (x[0], window_order.get(x[1], 99))):
            ...
            target = next((r for r in grp if r["component"] == "d_mixed_cz_daw_dz2_dt"), None)
            f.write(f"## {case_name} window={window}\n\n")
            if target is not None:
                f.write(f"- daw_dz2_rms_abs_rate: `{fmt(target['rms_abs_rate'])}`\n")
                f.write(f"- bulk_rms_abs_rate: `{fmt(target['bulk_rms_abs_rate'])}`\n\n")
            ranked = [r for r in grp if r["component"] != "d_mixed_cz_daw_dz2_dt"]
            ranked.sort(key=lambda r: abs(r["share_of_target_rms"]) if math.isfinite(r["share_of_target_rms"]) else -1.0, reverse=True)
            f.write("| component | rms_abs_rate | share_of_daw_dz2_rms | share_of_bulk_rms | corr_with_daw_dz2 | corr_with_bulk | max_abs_rate | final_rate |\n")
            f.write("|:---|---:|---:|---:|---:|---:|---:|---:|\n")
            for row in ranked:
                f.write(
                    f"| {row['component']} | {fmt(row['rms_abs_rate'])} | {fmt(row['share_of_target_rms'])} | {fmt(row['share_of_bulk_rms'])} | {fmt(row['corr_with_target'])} | {fmt(row['corr_with_bulk'])} | {fmt(row['max_abs_rate'])} | {fmt(row['final_rate'])} |\n"
                )
            f.write("\n")
