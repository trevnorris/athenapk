...
OUTPUT_ROOT="/projects/fluid-engine/out/step140_mode0_closure_term_isolation_local"
...
usage() {
  cat <<'USAGE'
Run Step-140 mode-0 closure/ledger term-isolation sweep.

Goal:
  Starting from Step-139 bounded controlled + active full baseline, isolate
  which source-term families are dominating:
    - local mode-0 closure failure
    - transport closure failure
    - EM bulk ledger failure
  while preserving transfer-floor activation.

Usage:
  ./scripts/run_step140_mode0_closure_term_isolation.sh [options]
...
echo ""
echo "Step-140 mode-0 closure/ledger term isolation complete."
