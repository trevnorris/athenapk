...
OUTPUT_ROOT="/projects/fluid-engine/out/step190_cx_cz_exact_delta_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-190 exact Cx/Cz discrete energy-increment probe on the corrected quadrature baseline.

Goal:
  Compare exact discrete Cx/Cz energy increments against the surviving EM bulk
  ledger residual on the baseline_quadrature case.

Usage:
  ./scripts/run_step190_cx_cz_exact_delta_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step190_cx_cz_exact_delta_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_cx_cz_exact_delta.csv" \
  --out-md "${OUTPUT_ROOT}_cx_cz_exact_delta.md"
...
echo "cx_cz_exact_delta_md,${OUTPUT_ROOT}_cx_cz_exact_delta.md"
