...
OUTPUT_ROOT="/projects/fluid-engine/out/step147_transport_ledger_isolation_local"
...
usage() {
  cat <<'USAGE'
Run Step-147 transport/ledger isolation after Step-146 local-closure fix.

Goal:
  Keep continuity-consistent current projection enabled on the full branch
  (local mode-0 closure fixed), then isolate the remaining transport + ledger
  blocker terms via compact A/B toggles.
...
Case matrix:
  ...
  - no_momw_sources
  - no_rho_divj
  - no_pressure_transport
  - no_pressure_rusanov

Usage:
  ./scripts/run_step147_transport_ledger_isolation.sh [options]
...
run_case "baseline_proj_full"
run_case "no_current" "modes4d/em_source_current_gain=0.0"
run_case "no_timelike" "modes4d/em_source_timelike_gain=0.0"
run_case "no_damping" "modes4d/em_source_damping_gain=0.0"
run_case "no_momw_sources" \
  "modes4d/plasma_momw_source_gain=0.0" \
  "modes4d/plasma_momw_pressure_source_gain=0.0"
run_case "no_rho_divj" "modes4d/plasma_rho_divj_gain=0.0"
run_case "no_pressure_transport" "modes4d/plasma_pressure_transport_gain=0.0"
run_case "no_pressure_rusanov" "modes4d/plasma_pressure_rusanov_gain=0.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step132_solver_equivalence.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --baseline-case "baseline_proj_full" \
...
echo ""
echo "Step-147 transport/ledger isolation complete."
