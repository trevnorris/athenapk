#!/usr/bin/env python3
"""Summarize scalar/mixed-channel activation probe outputs."""
...
def main():
    ...
    parser.add_argument(
        "--glob",
        default="/projects/fluid-engine/out/step105_scalar_channel_activation_*/*",
        help="Glob for output case directories",
    )
    parser.add_argument(
        "--out-csv",
        default="/projects/fluid-engine/out/step105_scalar_channel_activation_summary.csv",
        help="Path to write summary CSV",
    )
    parser.add_argument(
        "--out-md",
        default="/projects/fluid-engine/out/step105_scalar_channel_activation_summary.md",
        ...
        rows.append(
            {
                ...
                "piw_mode1_amp": parse_arg_from_log(log_path, "problem/harris_4d/piw_mode1_amp"),
                "ay_mode1_amp": parse_arg_from_log(log_path, "problem/harris_4d/ay_mode1_amp"),
                "piy_mode1_amp": parse_arg_from_log(log_path, "problem/harris_4d/piy_mode1_amp"),
                ...
                "full_max_abs_ew_l1": series_max_abs(full_cols.get("m4d_ew_l1")),
                "full_final_mixed_c2": full.get("final_mixed_c2", math.nan),
                "full_max_abs_mixed_c2": series_max_abs(full_cols.get("m4d_mixed_c2")),
                ...
                "full_max_abs_emf_cov_vxb_abs": series_max_abs(
                    full_cols.get("m4d_emf_cov_vxb_abs")
                ),
    ...
    with out_md.open("w", encoding="utf-8") as f:
        f.write("# Scalar/Mixed Channel Activation Summary\n\n")
        f.write(f"Parsed `{len(rows)}` cases from glob `{args.glob}`.\n\n")
        if not rows:
            f.write("No valid cases found.\n")
        else:
            f.write(
                "| case | drift_momw | aw/piw | ay/piy | jw_l1(final/max) | ew_l1(final/max) | mixed_c2(final/max) | jw_ew(final/max) | s_leak_abs(final/max) | emf_vw_c(final/max) | emf_cov(final/max) | psi_w0-psi_proj | psi0 ratio | closure_proj | closure_local | transport | ledger |\n"
            )
            f.write(
                "|:---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---|:---|:---|:---|\n"
            )
