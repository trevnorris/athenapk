...
OUTPUT_ROOT="/projects/fluid-engine/out/step211_postpatch_lowamp_threshold_local"
...
BASE_MODE1_AMP="1.25e-2"
BASE_MODE2_AMP="7.5e-3"
MODE1_OMEGA="6.283185307179586"
MODE2_OMEGA="12.566370614359172"

usage() {
  cat <<'USAGE'
Run Step-211 near-threshold post-patch amplitude sweep around the low-amplitude 2pi forcing case.
USAGE
}
...
FULL_BASE_ARGS=(
  ...
  "modes4d/response_w0_local_gate_t_on=0.0"
  "modes4d/response_w0_local_gate_t_off=-1.0"
  "modes4d/response_w0_local_gate_period=0.0"
  "modes4d/response_w0_local_gate_duty=1.0"
)
...
run_case "patched_static_screen" "0.0" "0.0"
run_case "patched_omega_2pi_amp040" "5.0e-3" "3.0e-3"
run_case "patched_omega_2pi_amp060" "7.5e-3" "4.5e-3"
run_case "patched_omega_2pi_amp080" "1.0e-2" "6.0e-3"
run_case "patched_omega_2pi_amp100" "${BASE_MODE1_AMP}" "${BASE_MODE2_AMP}"
...
python3 "${REPO_ROOT}/scripts/summarize_step211_postpatch_lowamp_threshold_sweep.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --summary-csv "${OUTPUT_ROOT}_summary.csv" \
  --phase2-csv "${OUTPUT_ROOT}_phase2.csv" \
  --out-csv "${OUTPUT_ROOT}_threshold_sweep.csv" \
  --out-md "${OUTPUT_ROOT}_threshold_sweep.md"
...
printf 'threshold_sweep_md,%s\n' "${OUTPUT_ROOT}_threshold_sweep.md"
