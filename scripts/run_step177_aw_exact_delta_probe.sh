...
OUTPUT_ROOT="/projects/fluid-engine/out/step177_aw_exact_delta_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-177 exact discrete A_w gradient-energy probe on the corrected quadrature baseline.

Goal:
  Test whether the surviving quadrature-baseline ledger defect is explained by the exact
  discrete gradient-energy increment of the A_w,z channel, including quadratic update terms.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step177_aw_exact_delta_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step177_aw_exact_delta_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_aw_exact_delta.csv" \
  --out-md "${OUTPUT_ROOT}_aw_exact_delta.md"
...
echo "aw_exact_delta_md,${OUTPUT_ROOT}_aw_exact_delta.md"
