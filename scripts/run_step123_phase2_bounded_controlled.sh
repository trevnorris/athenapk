...
OUTPUT_ROOT="/projects/fluid-engine/out/step123_phase2_bounded_controlled_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-123 Phase-2 bounded-controlled validation campaign.

Goal:
  Keep Step-122 active-transfer forcing, but shorten horizon to avoid controlled
  branch runaway while preserving transfer-floor activation.

Usage:
  ./scripts/run_step123_phase2_bounded_controlled.sh [options]
...
Options:
  ...
  --nlim INT               Run nlim (default: 35000)
  -h, --help               Show help
USAGE
}
...
echo ""
echo "Step-123 phase-2 bounded-controlled probe complete."
