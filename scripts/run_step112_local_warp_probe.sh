...
OUTPUT_ROOT="/projects/fluid-engine/out/step112_local_warp_probe_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-112 low-rank local warp probe.

Goal:
  Test the next gpt-response12 phase: local warp w0(x,t) (fixed projection
  window) with dynamic mode mixing, using a low-rank Fourier representation.

Usage:
  ./scripts/run_step112_local_warp_probe.sh [options]
...
# Control: no local warp
run_case "w0_local_nomix" \
  "modes4d/response_w0_local_enable=false"

# Single local mode (low rank)
run_case "w0_local_mode1" \
  ...
  "modes4d/response_w0_local_mode2_amp=0.0"

# Dual local modes (low-rank extension)
run_case "w0_local_dual" \
  "modes4d/response_w0_local_enable=true" \
  "modes4d/response_w0_local_mode1_amp=5.0e-2" \
  "modes4d/response_w0_local_mode1_kx=3.141592653589793" \
  "modes4d/response_w0_local_mode1_kz=6.283185307179586" \
...
python3 "${REPO_ROOT}/scripts/summarize_local_warp_probe.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_local_warp.csv" \
  --out-md "${OUTPUT_ROOT}_local_warp.md"

echo ""
echo "Step-112 low-rank local warp probe complete."
echo "output_root,${OUTPUT_ROOT}"
echo "summary_md,${OUTPUT_ROOT}_summary.md"
echo "projected_closure_md,${OUTPUT_ROOT}_projected_closure.md"
echo "local_warp_md,${OUTPUT_ROOT}_local_warp.md"
