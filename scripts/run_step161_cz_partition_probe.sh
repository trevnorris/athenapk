...
OUTPUT_ROOT="/projects/fluid-engine/out/step161_cz_partition_probe_local"
...
usage() {
  cat <<'USAGE'
Run Step-161 Cz partition probe on the corrected quadrature baseline.

Goal:
  Split the dominant Cz energy channel into daw_dz^2, dw_az^2, and cross terms
  to identify which Cz bookkeeping piece is carrying the remaining EM bulk ledger drift.

Case matrix:
  - baseline_quadrature

Usage:
  ./scripts/run_step161_cz_partition_probe.sh [options]
USAGE
}
...
python3 "${REPO_ROOT}/scripts/summarize_step161_cz_partition.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --out-csv "${OUTPUT_ROOT}_cz_partition.csv" \
  --out-md "${OUTPUT_ROOT}_cz_partition.md"
...
echo "cz_partition_md,${OUTPUT_ROOT}_cz_partition.md"
