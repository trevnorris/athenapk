...
OUTPUT_ROOT="/projects/fluid-engine/out/step126_controlled_source_pinning_audit_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'USAGE'
Run Step-126 controlled-source pinning audit.

Goal:
  Keep the full branch fixed in the known active configuration (Step-124 t100
  plus-force), and sweep controlled-only source-family toggles to identify which
  source families drive controlled runaway at tlim=1.00.

Usage:
  ./scripts/run_step126_controlled_source_pinning_audit.sh [options]
...
# Keep full branch anchored in the transfer-active Step-124 t100_plus_force setup.
...
# Baseline at the Step-124 t100 horizon.
run_case "ctrl_baseline"

# Per-source-family controlled toggles.
run_case "ctrl_no_em_current" \
  "modes4d/em_source_current_gain=0.0"

run_case "ctrl_no_em_timelike" \
  "modes4d/em_source_timelike_gain=0.0"

run_case "ctrl_no_momw_source" \
  "modes4d/plasma_momw_source_gain=0.0"

run_case "ctrl_no_momw_pressure" \
  "modes4d/plasma_momw_pressure_source_gain=0.0"

run_case "ctrl_no_plasma_force" \
  "modes4d/plasma_force_source_gain=0.0"

run_case "ctrl_no_plasma_energy" \
  "modes4d/plasma_energy_source_gain=0.0"

# Combined clamp references.
run_case "ctrl_all_sources_zero" \
  "modes4d/em_source_current_gain=0.0" \
  "modes4d/em_source_timelike_gain=0.0" \
  "modes4d/plasma_momw_source_gain=0.0" \
  "modes4d/plasma_momw_pressure_source_gain=0.0" \
...
run_case "ctrl_all_sources_zero_damped" \
  "modes4d/em_source_current_gain=0.0" \
  "modes4d/em_source_timelike_gain=0.0" \
  "modes4d/plasma_momw_source_gain=0.0" \
  "modes4d/plasma_momw_pressure_source_gain=0.0" \
  "modes4d/plasma_force_source_gain=0.0" \
  "modes4d/plasma_energy_source_gain=0.0" \
  "modes4d/em_damping=5.0e-1" \
  "modes4d/plasma_momw_damping=2.0"
...
python3 "${REPO_ROOT}/scripts/summarize_step126_controlled_source_pinning.py" \
  --glob "${OUTPUT_ROOT}/*" \
  --controlled-bounds-csv "${OUTPUT_ROOT}_controlled_bounds.csv" \
  --baseline-case "ctrl_baseline" \
  --out-csv "${OUTPUT_ROOT}_source_pinning.csv" \
  --out-md "${OUTPUT_ROOT}_source_pinning.md"

echo ""
echo "Step-126 controlled-source pinning audit complete."
...
echo "source_pinning_md,${OUTPUT_ROOT}_source_pinning.md"
