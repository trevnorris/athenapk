#!/usr/bin/env python3
"""Compare bulk residual against all-mode local-gradient response-work sums."""
...
def main():
    ...
    for case_dir in case_dirs:
        ...
        components = {
            'local_gradient_mix_x_sum': select_window(t_r, derivative_series(times, cols['m4d_int_aw_gradx_power_mix_w0_local_gradient_discrete_sum'])[1], t_lo, end, include_right=True),
            'local_gradient_mix_z_sum': select_window(t_r, derivative_series(times, cols['m4d_int_aw_gradz_power_mix_w0_local_gradient_discrete_sum'])[1], t_lo, end, include_right=True),
        }
        components['local_gradient_mix_xz_sum'] = sum_series(components['local_gradient_mix_x_sum'], components['local_gradient_mix_z_sum'])
        components['full_da_dt_xz_sum'] = select_window(
            t_r,
            sum_series(
        ...
        rows.append({
            'case_name': case_dir.name, 'component': 'em_bulk_ledger_residual', 'rms_abs_rate': res_rms,
            'corr_with_residual': 1.0, 'fit_coeff': 1.0, 'corrected_rms_abs_rate': 0.0,
        ...
        for name, vals in components.items():
            ...
            rows.append({
                'case_name': case_dir.name, 'component': name, 'rms_abs_rate': rms_abs,
                'corr_with_residual': pearson(vals, y), 'fit_coeff': alpha,
                'corrected_rms_abs_rate': corrected_rms, 'rms_reduction_factor': reduction,
            })

    if not rows:
        raise SystemExit('No Step-182 rows were computed.')
    ...
    with out_md.open('w', encoding='utf-8') as f:
        f.write('# Step-182 Local-Gradient Work Sum Probe\n\n')
        ...
        for case_name in sorted({r['case_name'] for r in rows}):
            ...
            for r in ranked:
                f.write(f"| {r['component']} | {fmt(r['rms_abs_rate'])} | {fmt(r['corr_with_residual'])} | {fmt(r['fit_coeff'])} | {fmt(r['corrected_rms_abs_rate'])} | {fmt(r['rms_reduction_factor'])} |\n")
            f.write('\n')
